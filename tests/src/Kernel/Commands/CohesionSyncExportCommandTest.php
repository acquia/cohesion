...
class CohesionSyncExportCommandTest extends EntityKernelTestBase {
  ...
  protected function setUp() {
    ...
    $this->command = new CohesionSyncExportCommand($this->container->get('entity_type.manager'), $this->configStorage, $cohesionFullStorage, $config_manager);
  }

  /**
   * Test site studio export command.
   *
   * @covers \Drupal\cohesion_sync\Commands\CohesionSyncExportCommand::siteStudioExport
   */
  public function testSiteStudioExport() {
    ...
    $this->assertEqual(Yaml::decode(file_get_contents($path . '/' . $template_name . '.yml')), $template_data);

    // Component.
    $this->assertFileExists($path . '/' . $component_name . '.yml', 'Component yml should exist');
    $this->assertEqual(Yaml::decode(file_get_contents($path . '/' . $component_name . '.yml')), $component_data);
    // Component dependency file.
    $this->assertFileExists($path . '/' . $filename, 'File dependency should exist');
    $this->assertEqual(file_get_contents($path . '/' . $filename), $contents);

    // Component with config dependencies.
    $this->assertFileExists($path . '/' . $component_config_dep_name . '.yml', 'Component yml should exist');
    $this->assertEqual(Yaml::decode(file_get_contents($path . '/' . $component_config_dep_name . '.yml')), $component_config_dep_data);
    // Context.
    $this->assertFileNotExists($path . '/' . $context_name . '.yml', 'Context yml not should exist');
    // Image style.
    $this->assertFileExists($path . '/' . $image_style_name . '.yml', 'Image style yml should exist as it is a dependency of a SS config');
    $this->assertEqual(Yaml::decode(file_get_contents($path . '/' . $image_style_name . '.yml')), $image_style_data);
