...
class CohesionApiClientTest extends UnitTestCase {
  ...
  protected function setUp(): void {
    ...
    // 'wait' is mocked out so retry tests don't incur a real delay.
    $this->cohesionApiClient = $this->getMockBuilder(CohesionApiClient::class)
      ->onlyMethods(['requestHeaders', 'wait'])
      ->getMock();
  ...
  public static function exceptionHandlingProvider(): iterable {
    ...
    // Non-JSON body (e.g. an HTML error page). Json::decode() throws
    // \InvalidArgumentException for this input, so the handler must decode
    // natively and fall back to the stripped raw body for 'error'.
    $htmlResponse = new Response(502, [], '<html><body>Bad Gateway</body></html>');
    $htmlRequest = new Request('GET', '/test');
    $htmlException = new RequestException('Bad gateway', $htmlRequest, $htmlResponse);

    yield 'RequestException with non-JSON HTML body' => [
      $htmlException,
      ['error' => 'Bad Gateway'],
      1,
      0,
    ...
    // Empty body. Json::decode() throws \InvalidArgumentException for this
    // input; the 'error' offset must still be set (to an empty string) rather
    // than triggering a decode exception or an undefined-offset error.
    $emptyResponse = new Response(500, [], '');
    $emptyRequest = new Request('GET', '/test');
    $emptyException = new RequestException('Server error', $emptyRequest, $emptyResponse);

    yield 'RequestException with empty body' => [
      $emptyException,
      ['error' => ''],
      1,
      0,
    ...
    $timeoutRequest = new Request('GET', '/test');
    $timeoutException = new RequestException('Read timeout', $timeoutRequest);

    // No response (e.g. a timeout after the request was sent) must not be
    // retried: the server may have already applied a non-idempotent write.
    yield 'RequestException without response body' => [
      $timeoutException,
      'Read timeout',
      1,
      0,
    ...
    // The connection never established, so nothing reached the server -
   ...
   */
  public function testSendCatchesExceptions(\Exception $exception, $expected, int $expectedRequestCalls, int $expectedWaitCalls): void {
    $this->httpClient->expects($this->exactly($expectedRequestCalls))
      ->method('request')
      ->willThrowException($exception);

    $this->cohesionApiClient->expects($this->exactly($expectedWaitCalls))
      ->method('wait');
