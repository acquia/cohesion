...
class CohesionLayoutRevisionManagerTest extends UnitTestCase {
  ...
  /**
   * @covers ::batchDeleteRevisions
   * @covers ::purgeStrayFieldRevisionRow
   */
  public function testBatchDeleteRevisionsPurgesStrayFieldRevisionRow(): void {
    $storage = $this->createMock(RevisionableStorageInterface::class);
    $this->entityTypeManager->method('getStorage')->willReturn($storage);
    $storage->method('loadRevision')->willReturn(NULL);
    $storage->expects($this->never())->method('deleteRevision');

    // No matching row in cohesion_layout_revision — confirms it's stray.
    $existsSelectMock = $this->buildSelectMock([], 0);
    $this->database->expects($this->once())->method('select')
      ->with('cohesion_layout_revision', 'clr')
      ->willReturn($existsSelectMock);

    $deleteMock = $this->createMock(\Drupal\Core\Database\Query\Delete::class);
    $deleteMock->method('condition')->willReturnSelf();
    $deleteMock->method('execute')->willReturn(1);
    $this->database->expects($this->once())->method('delete')
      ->with('cohesion_layout_field_revision')
      ->willReturn($deleteMock);

    $this->assertEquals(1, $this->revisionManager->batchDeleteRevisions([5]));
  }
   ...
   * @covers ::purgeStrayFieldRevisionRow
   */
  public function testBatchDeleteRevisionsDoesNotPurgeWhenRevisionRowExists(): void {
    $storage = $this->createMock(RevisionableStorageInterface::class);
    $this->entityTypeManager->method('getStorage')->willReturn($storage);
    $storage->method('loadRevision')->willReturn(NULL);
    $storage->expects($this->never())->method('deleteRevision');

    // A matching row exists — loadRevision() failed for some other reason,
    // so the row must not be touched.
    $existsSelectMock = $this->buildSelectMock([], 1);
   ...
   * @covers ::purgeStrayFieldRevisionRow
   */
  public function testBatchDeleteRevisionsPurgeHandlesExceptions(): void {
    $storage = $this->createMock(RevisionableStorageInterface::class);
    $this->entityTypeManager->method('getStorage')->willReturn($storage);
    $storage->method('loadRevision')->willReturn(NULL);
    $storage->expects($this->never())->method('deleteRevision');
   ...
   * @covers ::purgeStrayFieldRevisionRow
   */
  public function testBatchDeleteRevisionsMixedBatchAccumulatesProgress(): void {
    $storage = $this->createMock(RevisionableStorageInterface::class);
    $this->entityTypeManager->method('getStorage')->willReturn($storage);

    $liveRevision = $this->createMock(RevisionableInterface::class);
    $liveRevision->method('isDefaultRevision')->willReturn(FALSE);
    $liveRevision->method('getEntityTypeId')->willReturn('cohesion_layout');

    // Revision 1 loads normally; revision 2 is a stray row.
    $storage->method('loadRevision')->willReturnCallback(
      fn ($id) => $id === 1 ? $liveRevision : NULL
    );
    $storage->expects($this->once())->method('deleteRevision')->with(1);
    ...
    $this->assertEquals(2, $this->revisionManager->batchDeleteRevisions([1, 2]));
  }
