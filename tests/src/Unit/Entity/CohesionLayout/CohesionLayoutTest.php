...
TWIG;
   ...
   *
   * @return array
   *   Test cases with langcode, entity_id, cache_entity_id, cached_langcode,
   *   and expected_apply.
   */
   ...
   * @param int $cache_entity_id
   *   The entity ID used as the cache key.
   * @param string $cached_langcode
   *   The language code used in the translation cache.
   * @param bool $expected_apply
   *   Whether the cached translation should be applied.
   *
   * @covers ::preSave
   * @dataProvider tmgmtTranslationCacheDataProvider
   */
  public function testPreSaveTmgmtTranslationCache(string $langcode, ?int $entity_id, int $cache_entity_id, string $cached_langcode, bool $expected_apply): void {
    $cachedJson = '{"canvas":[{"type":"translated_component","lang":"' . $cached_langcode . '"}]}';

    $translation_cache = &drupal_static('cohesion_layout_tmgmt_translations', []);
    $translation_cache[$cache_entity_id][$cached_langcode] = $cachedJson;
