...
namespace Drupal\Tests\cohesion\Config\Unit;
 ...
 * @group Cohesion
 *
 * @package Drupal\Tests\cohesion\Config\Unit
 */
class CohesionFullPackageStorageTest extends UnitTestCase {
  ...
  public function setUp() {
    ...
    $this->configStorage->expects($this->any())
      ...
      ->willReturnCallback(function ($name) {
        return isset($this->configs[$name]['exists']) ? $this->configs[$name]['exists'] : TRUE;
      });

    $this->configStorage->expects($this->any())
      ->method('read')
      ->willReturnCallback(function ($name) {
        return isset($this->configs[$name]['data']) ? $this->configs[$name]['data'] : FALSE;
      });

    $this->configManager->expects($this->any())
      ->method('getEntityTypeIdByName')
      ->willReturnCallback(function ($name) {
        return $this->configs[$name]['entity_type'];
      });
  ...
  public function testBuildDependencies() {
    ...
    $this->assertContains('config_dependency_1', $this->storage->listAll(), 'List all shout contain config dependency 1');
    $this->assertContains('config_dependency_2', $this->storage->listAll(), 'List all shout contain config dependency 2');
    $this->assertNotContains('module_dependency_1', $this->storage->listAll(), 'List all shout contain module dependency');
    $this->assertNotContains('file:file:dependency_1', $this->storage->listAll(), 'List all shout contain content dependency');
  }
  ...
  public function testConfigStatus() {
    ...
    $this->assertContains('some_module.config.without_status', $this->storage->listAll(), 'List all shout contain config with no status');
    $this->assertContains('some_module.config.with_status_true', $this->storage->listAll(), 'List all shout contain config with status TRUE');
    $this->assertNotContains('some_module.config.with_status_false', $this->storage->listAll(), 'List all should not container config with status FALSE');
  }

  /**
   * Test the getStorageFileList
   *
