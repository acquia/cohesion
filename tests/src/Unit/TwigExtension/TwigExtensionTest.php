...
/**
 * Tests for TwigExtension (block rendering, cache metadata, permissions).
 *
 * @group Cohesion
 */
class TwigExtensionTest extends UnitTestCase {
  ...
  protected function setUp(): void {
    ...
    $cohesion_utils_real = new \Drupal\cohesion\Services\CohesionUtils(
      ...
      $this->createMock('\\Drupal\\Core\\Utility\\Token')
    );
    $cohesion_utils = $this->createMock('\\Drupal\\cohesion\\Services\\CohesionUtils');
    $cohesion_utils->method('escapeExecutableHtml')->willReturnCallback(
      fn (string $html) => $cohesion_utils_real->escapeExecutableHtml($html)
    );
  ...
  /**
   * Tests that dx8_hidden blocks use a lazy_builder to defer rendering.
   */
  public function testDrupalBlockDx8HiddenUsesLazyBuilder(): void {
    $access = $this->mockAccess(
      '\Drupal\Core\Access\AccessResultAllowed',
      TRUE,
      ['user'],
    ...
    $block = $this->createMock('\Drupal\block\Entity\Block');
    $block->method('access')->willReturn($access);
    $block->method('id')->willReturn('test_block');
    $block->method('getRegion')->willReturn('dx8_hidden');

    $this->mockEntityTypeManager($block);

    $result = $this->twigExtension->drupalBlock('test_block');

    // Result uses a lazy_builder — no direct #markup or #cache from the block.
    $this->assertArrayHasKey('#lazy_builder', $result);
    $this->assertArrayNotHasKey('#markup', $result);

    // Lazy builder callback and argument are correct.
    $this->assertEquals(
      TwigExtension::class . '::lazyBuildHiddenBlock',
      $result['#lazy_builder'][0]
    );
    $this->assertEquals(['test_block'], $result['#lazy_builder'][1]);

    // Access check cacheability IS applied to the build.
    $this->assertContains('config:block.block.test_block', $result['#cache']['tags'] ?? []);
    $this->assertContains('user', $result['#cache']['contexts'] ?? []);
  }

  /**
   * Tests lazyBuildHiddenBlock renders in isolation with correct output.
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::lazyBuildHiddenBlock
   */
  public function testLazyBuildHiddenBlockRendersIsolated(): void {
    $blockBuild = [
      '#markup' => 'Block content',
      '#cache' => [
        'contexts' => ['session'],
        'tags' => ['config:views.view.some_view'],
        'max-age' => 0,
      ],
      '#attached' => [
        'library' => ['cohesion/some_block_library'],
      ],
    ];
    $viewBuilder = $this->createViewBuilder($blockBuild);

    $block = $this->createMock('\Drupal\block\Entity\Block');
    $storage = $this->createMock('\Drupal\Core\Entity\EntityStorageInterface');
    $storage->method('load')->with('test_block')->willReturn($block);

    $renderer = $this->createMock('\Drupal\Core\Render\RendererInterface');
    $render_method = method_exists($renderer, 'renderInIsolation') ? 'renderInIsolation' : 'renderPlain';
    $renderer->method($render_method)
      ->willReturn('<div>Rendered block</div>');

    $container = \Drupal::getContainer();
    $newContainer = $this->createMock(ContainerInterface::class);
    $newContainer->method('get')->willReturnCallback(function ($id) use ($storage, $viewBuilder, $renderer) {
      return match ($id) {
        'entity_type.manager' => $this->createEntityTypeManagerMock($storage, $viewBuilder),
        'renderer' => $renderer,
        default => NULL,
      };
    });
    $newContainer->method('has')->willReturnCallback(fn ($id) => in_array($id, ['entity_type.manager', 'renderer'], TRUE));
    \Drupal::setContainer($newContainer);

    try {
      $result = TwigExtension::lazyBuildHiddenBlock('test_block');

      // HTML is wrapped in Markup::create to skip XSS filtering.
      $this->assertArrayHasKey('#markup', $result);
      $this->assertInstanceOf('\Drupal\Core\Render\Markup', $result['#markup']);
      $this->assertEquals('<div>Rendered block</div>', (string) $result['#markup']);

      // Block's #attached libraries are preserved so CSS/JS still loads.
      $this->assertEquals(['cohesion/some_block_library'], $result['#attached']['library'] ?? []);

      // The block's full cache metadata bubbles into the result: this array
      // is the #lazy_builder placeholder replacement, so it is the only
      // #cache core sees. Dropping contexts/max-age here would let output
      // that actually varies by session, or is genuinely uncacheable, be
      // stored in the page/CDN cache as if it were static — e.g. a CSRF
      // token or user-specific output served back to a different user.
      $this->assertEquals(['config:views.view.some_view'], $result['#cache']['tags'] ?? []);
      $this->assertEquals(['session'], $result['#cache']['contexts'] ?? []);
      $this->assertSame(0, $result['#cache']['max-age'] ?? NULL);
    }
    finally {
      \Drupal::setContainer($container);
    }
  ...
  /**
   * Tests lazyBuildHiddenBlock returns empty markup for nonexistent block.
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::lazyBuildHiddenBlock
   */
  public function testLazyBuildHiddenBlockNonexistentBlock(): void {
    $container = \Drupal::getContainer();
    $storage = $this->createMock('\Drupal\Core\Entity\EntityStorageInterface');
    $storage->method('load')->willReturn(NULL);

    $newContainer = $this->createMock(ContainerInterface::class);
    $newContainer->method('get')->willReturnCallback(function ($id) use ($storage) {
      return match ($id) {
        'entity_type.manager' => $this->createEntityTypeManagerMock($storage),
        default => NULL,
      };
    });
    $newContainer->method('has')->willReturnCallback(fn ($id) => $id === 'entity_type.manager');
    \Drupal::setContainer($newContainer);

    try {
      $result = TwigExtension::lazyBuildHiddenBlock('nonexistent');
      $this->assertArrayHasKey('#markup', $result);
  ...
  /**
   * Tests that lazyBuildHiddenBlock is registered as a trusted callback.
   *
   * Drupal's renderer rejects #lazy_builder callbacks from classes that
   * do not implement TrustedCallbackInterface. Without this, the page
   * would 500 at runtime.
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::trustedCallbacks
   */
  public function testLazyBuildHiddenBlockIsTrustedCallback(): void {
    $this->assertInstanceOf(
      'Drupal\Core\Security\TrustedCallbackInterface',
      $this->twigExtension
    );
    $callbacks = TwigExtension::trustedCallbacks();
    $this->assertContains('lazyBuildHiddenBlock', $callbacks);
  }
  ...
  /**
   * Creates a standalone entity type manager mock for use in static method tests.
   *
   * @param \Drupal\Core\Entity\EntityStorageInterface $storage
   *   The storage mock.
   * @param \Drupal\Core\Entity\EntityViewBuilderInterface|null $view_builder
   *   Optional view builder mock.
   *
   * @return \Drupal\Core\Entity\EntityTypeManagerInterface
   *   The entity type manager mock.
   */
  private function createEntityTypeManagerMock($storage, $view_builder = NULL) {
    $manager = $this->createMock('\Drupal\Core\Entity\EntityTypeManagerInterface');
    $manager->method('getStorage')->willReturn($storage);
    if ($view_builder) {
      $manager->method('getViewBuilder')->willReturn($view_builder);
  ...
  public static function tokenReplaceAmpersandProvider(): array {
    return [
      ...
      'script tags should always be escaped in template context' => [
        '<script>alert("xss")</script>',
        '&lt;script&gt;alert("xss")&lt;/script&gt;',
        TRUE,
      ],
      ...
      'p tags not escaped in template context' => [
        '<p>a wysiwyg with content</p>',
        '<p>a wysiwyg with content</p>',
        TRUE,
      ],
      'onerror attribute not escaped in layout canvas (escaping deferred)' => [
        '<img src=x onerror=alert(1)>',
        '<img src=x onerror=alert(1)>',
        FALSE,
      ],
      'unquoted onerror attribute escaped in template context' => [
        '<img src=x onerror=alert(1)>',
        '&lt;img src=x onerror=alert(1)&gt;',
        TRUE,
      ],
      'quoted onload attribute escaped in template context' => [
        '<svg onload="alert(1)"></svg>',
        '&lt;svg onload="alert(1)"&gt;&lt;/svg&gt;',
        TRUE,
      ],
      'javascript uri in href escaped in template context' => [
        '<a href="javascript:alert(1)">Click</a>',
        '&lt;a href="javascript:alert(1)"&gt;Click&lt;/a&gt;',
        TRUE,
      ],
      'iframe tags escaped in template context' => [
        '<iframe src="//evil.com"></iframe>',
        '&lt;iframe src="//evil.com"&gt;&lt;/iframe&gt;',
        TRUE,
      ],
      'ampersand preserved alongside escaped tag in template context' => [
  ...
  /**
   * Tests tokenReplace skips entity resolution for non-entity-backed menu
   * links (e.g. taxonomy_menu, Views) instead of throwing an unhandled
   * PluginNotFoundException that floods the watchdog log (ACMS-6399).
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::tokenReplace
   */
  public function testTokenReplaceSkipsNonEntityBackedMenuLink(): void {
    $entityTypeManager = $this->createMock('\Drupal\Core\Entity\EntityTypeManagerInterface');
    $entityTypeManager->expects($this->once())
      ->method('hasDefinition')
      ->with('taxonomy_menu')
      ->willReturn(FALSE);
    $this->setProtectedProperty($this->twigExtension, 'entityTypeManager', $entityTypeManager);

    $entityRepository = $this->createMock('\Drupal\Core\Entity\EntityRepositoryInterface');
    $entityRepository->expects($this->never())->method('loadEntityByUuid');
    $this->setProtectedProperty($this->twigExtension, 'entityRepository', $entityRepository);

    $loggerChannel = $this->createMock('\Drupal\Core\Logger\LoggerChannelInterface');
    $loggerChannel->expects($this->never())->method('error');
    $this->setProtectedProperty($this->twigExtension, 'loggerChannel', $loggerChannel);

    $token = $this->createMock('\Drupal\Core\Utility\Token');
    $token->method('replace')->willReturnArgument(0);
    $this->setProtectedProperty($this->twigExtension, 'token', $token);

    $originalLink = $this->createMock('\Drupal\Core\Menu\MenuLinkInterface');
    $originalLink->method('getPluginDefinition')->willReturn(['id' => 'taxonomy_menu:some-uuid']);

    $context = ['item' => ['original_link' => $originalLink]];
    $data = ['menu_link_content' => NULL];

    // Should not throw, and the token should still be replaced (as a plain
    // string token, since no entity could be resolved).
    $result = $this->withLanguageManagerContainer(
      fn () => $this->twigExtension->tokenReplace('[menu_link_content:title]', $data, $context)
    );
    $this->assertEquals('[menu_link_content:title]', (string) $result);
  }

  /**
   * Tests tokenReplace still resolves entity-backed menu links (regression
   * guard for ACMS-6399's guard clause).
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::tokenReplace
   */
  public function testTokenReplaceResolvesEntityBackedMenuLink(): void {
    $entity = $this->createMock('\Drupal\menu_link_content\Entity\MenuLinkContent');
    ...
    $entityRepository->expects($this->once())
      ...
      ->with('menu_link_content', 'some-uuid')
      ->willReturn($entity);
    $this->setProtectedProperty($this->twigExtension, 'entityRepository', $entityRepository);
    ...
    $token->expects($this->once())
      ->method('replace')
      ->with($this->anything(), $this->callback(function ($data) use ($entity) {
        return $data['menu_link_content'] === $entity;
      }), $this->anything())
      ->willReturnArgument(0);
    $this->setProtectedProperty($this->twigExtension, 'token', $token);

    $originalLink = $this->createMock('\Drupal\Core\Menu\MenuLinkInterface');
    $originalLink->method('getPluginDefinition')->willReturn(['id' => 'menu_link_content:some-uuid']);

    $context = ['item' => ['original_link' => $originalLink]];
    $data = ['menu_link_content' => NULL];

    $this->withLanguageManagerContainer(
      fn () => $this->twigExtension->tokenReplace('[menu_link_content:title]', $data, $context)
    );
  }

  /**
   * Runs a callback with a container providing a mocked language_manager
   * service (as tokenReplace() calls \Drupal::languageManager() directly),
   * restoring the original container afterwards.
   *
   * @param callable $callback
   *   Callback to invoke with the swapped container in place.
   *
   * @return mixed
   *   The callback's return value.
   */
  private function withLanguageManagerContainer(callable $callback) {
    $languageManager = $this->createMock('\Drupal\Core\Language\LanguageManagerInterface');
    $language = $this->createMock('\Drupal\Core\Language\LanguageInterface');
    $language->method('getId')->willReturn('en');
    $languageManager->method('getCurrentLanguage')->willReturn($language);
  ...
  /**
   * Two views must not share a counter.
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::getViewIterate
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::incrementViewIterate
   */
  ...
  /**
   * Two views with the same id() and display_id still get separate counters.
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::getViewIterate
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::incrementViewIterate
   */
  public function testViewIterateKeyIsUniquePerViewObject(): void {
    ...
    $view_b->method('id')->willReturn('view_a');
  ...
  /**
   * Counter works even with no dom_id, id() or display_id on the view.
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::getViewIterate
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::incrementViewIterate
   */
  public function testViewIterateWorksWithoutDomIdOrViewId(): void {
    $view = new \stdClass();
    $context = ['view' => $view];
    $this->twigExtension->incrementViewIterate($context);
  ...
  /**
   * Counter must survive Twig copying $context inside a {% block %}.
   *
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::getViewIterate
   * @covers \Drupal\cohesion_templates\TwigExtension\TwigExtension::incrementViewIterate
   */
  public function testViewIterateSurvivesContextCopy(): void {
    $context = ['view' => new \stdClass()];
    $this->twigExtension->setViewIterate($context);

    for ($i = 0; $i < 5; $i++) {
      // Simulate Twig passing a copy of $context into a block.
      $blockContext = $context;
      $this->assertEquals($i, $this->twigExtension->getViewIterate($blockContext));
      $this->twigExtension->incrementViewIterate($blockContext);
    }
  }
