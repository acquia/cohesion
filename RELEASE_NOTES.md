...
Run a Site Studio import to ensure the updated assets are loaded.

### Component tag synchronization and validation

#### What is it?

Enhanced component tag handling to ensure tags are kept in sync between component definitions and existing content. When component tags are added or removed, the system now automatically updates tag data on existing layout canvas entities that contain those components. Additionally, real-time validation displays an error toast notification when editing content if any child components in tagged dropzones do not have the required tags. This addresses an issue where component tags added or modified after content creation were not reflected in existing content, causing "This component cannot be dropped here" errors when attempting to reorder components.

#### What impact will there be?

- Component tag data is automatically synchronized on existing layout canvas entities when component tags are modified
- Users will immediately see an error toast message when loading content with tag mismatches between dropzone requirements and child components
- The error notification only appears once per page load, preventing notification spam
- Makes it easier to identify and resolve tag mismatch issues in existing content
- Improves user experience by proactively highlighting tag configuration problems and keeping content up-to-date

#### What actions do I need to take?

None. Tag synchronization and validation run automatically. When editing components with tagged dropzones, the system will ensure tag data is current. If you see the error toast, review your component tags to ensure child components have at least one tag that matches the dropzone requirements.

#### Are there any risks I should be aware of?

None. This is a non-breaking enhancement that synchronizes tag data and adds validation feedback. Existing content structure and functionality remain unchanged.

## Highlights

### React App Now Bundled with Module

#### What is it?

The React admin application is now bundled directly with the Drupal module instead of being hosted on the API. This change allows for faster issue resolution and the ability to apply patches directly to the React app when needed.

#### What impact will there be?

- **For End Users**: No visible changes; all functionality works as before
- **For Developers**: The React app source code is now version controlled with the module, enabling easier debugging and patching
...
**Warning**: Any custom modifications made to the React application files are unsupported and may cause unexpected behavior. Acquia accepts no responsibility for issues caused by custom changes to the React app source code.
...
Page Builder button will appear in the Navigation top bar when Navigation module is enabled.
