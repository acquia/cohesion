...
### Fixes stale content and a cache leak on pages using hidden-region blocks

#### What is it?

Blocks placed in Site Studio's hidden region (`dx8_hidden`) are rendered in isolation to prevent their cache metadata from leaking into the page cache.
This isolation was dropping the block's cache metadata (tags, contexts, and max-age) entirely. As a result:
- Cache tags (e.g. those tracking a Site Studio view template) never reached the page cache, so editing a view template did not invalidate node pages that rendered it through a hidden-region block, leaving those pages showing stale content.
- Cache contexts (e.g. `session`) and max-age were also lost, so output that actually varies per session/user, or that must not be cached at all, could be stored in the page or CDN cache and served back to a different user.

#### What impact will there be?

The full cache metadata (tags, contexts, and max-age) from blocks in the hidden region now bubbles up to the containing page. Pages using view templates via hidden-region blocks are invalidated correctly when the template changes, and pages containing hidden-region blocks that vary by session/user are no longer cached in a way that could leak content between users.
...
Pages containing a hidden-region block that requires session/user-specific rendering, or that declares a short max-age, may become less cacheable (or uncacheable) than before, since that variation now correctly bubbles up to the page. This is a deliberate correctness fix: the previous behavior could serve one user's session-specific content (e.g. a CSRF token) to another user from a shared page/CDN cache.

## 8.2.8

## Highlights

### Navigation module support (Drupal 11.3+)

#### What is it?

Site Studio Page Builder now supports Drupal 11.3's Navigation module. The Page Builder button now appears in the Navigation top bar, allowing users to access visual edit mode.

#### What impact will there be?

Page Builder button will appear in the Navigation top bar when Navigation module is enabled (Drupal 11.3+). On Drupal 10, the button continues to appear in the Admin Toolbar as before.
...
### Fixes an issue where the token browser was not showing all available tokens

#### What is it?

Fixed an issue where the token browser was not showing all available tokens in the token browser on components.
An unnecessary context restriction was being applied to the token browser, which was preventing some tokens from being displayed.

#### What impact will there be?

All tokens will now be displayed in the token browser on components as before.
...
### Fixes view rows repeating the first item when placed inside a component drop zone

#### What is it?

Fixed an issue where a View row rendered inside a component drop zone always showed the first result, repeated for every row, instead of iterating through the view's results.

#### What impact will there be?

Views rendered this way now display each row correctly.
...
### Fixes invalid tokens being passed to Site Studio API on rebuild

#### What is it?

When a module such as ECA was enabled and a rebuild was run errors would occur due to invalid tokens being passed to the Site Studio API.
The tokens are now validated before being passed to the API and any invalid tokens are not processed.

#### What impact will there be?

Using modules such as ECA will no longer cause errors on rebuild.
...
### Fixed an issue where uploading a component preview image would not update if one tag was attached to the component

#### What is it?

When a component had one tag attached, uploading a new preview image would not upload correctly, resulting in an AJAX error.

#### What impact will there be?

Users can now upload a new preview image for a component with one tag attached, without error.
...
### Fixes a fatal error when requesting translations via Translation Management (TMGMT)

#### What is it?

Fixed an issue where requesting a translation through the Translation Management (TMGMT) module could trigger a fatal error on content with an empty or uninitialized Layout Canvas field.

#### What impact will there be?

Requesting translations via TMGMT no longer fails with a fatal error, even when the underlying content has an empty or uninitialized Layout Canvas field.
...
### Video URL field in component form fails to retain empty state when cleared

#### What is it?

Fixed an issue where clearing a Video URL field (`MediaEmbed`) in a component form failed to retain its empty state because the default value was unintentionally re-applied on component re-render.

#### What impact will there be?

Content editors can now clear out the video URL from component forms and have the field remain empty without the default URL repopulating.
...
### Fixes the dx8-gateway service hanging when compiling certain styles

#### What is it?

Fixed two inputs that could lock up the `dx8-gateway` Node service indefinitely when posting to the `/build/style` endpoint. Both drove the CSS value parser (`postcss-values-parser`) into an infinite loop, which blocked the event loop and caused the request to time out:

- A font stack holding a Google Fonts stylesheet URL, because of the `|` characters in the URL.
- A style value containing a non-breaking space (U+00A0) next to a function, e.g. a `box-shadow`/`rgb()` value pasted from a rich-text editor.

#### What impact will there be?

Style compilation now completes for these payloads. A URL-shaped font stack (e.g. a Google Fonts stylesheet URL) is preserved verbatim as a quoted value, as per previous versions. Non-breaking spaces in style values are normalised to regular spaces before parsing.
...
Only affected inputs change behaviour: URL-shaped font stacks are emitted as a quoted value (previously would hang), and non-breaking spaces in values become regular spaces (which is how they render anyway). Output for all other styles is unchanged.

### Fixes the rebuild process not logging all entities being rebuilt

#### What is it?

Fixed an issue where the rebuild process was not logging all entities being rebuilt.
This has been fixed to ensure that all entities are logged during the rebuild process.

#### What impact will there be?

When running a rebuild via drush all entities being rebuilt will be logged.

#### What actions do I need to take?

Run a rebuild in verbose mode to see all logged information.

#### Are there any risks I should be aware of?

None.

### Style guide form field default value field is not visible

#### What is it?

Fixed an issue where the Default value field was not visible when editing form field elements in the Style guide builder.

#### What impact will there be?

Users can now set default values for Style guide form field elements as expected.

#### What actions do I need to take?

Site Studio import is required to ensure the updated JavaScript assets are loaded.

#### Are there any risks I should be aware of?

None.

### Fixes an issue where REST resource config could still exist

#### What is it?

When upgrading to 8.2.7, the REST resource plugins were removed because they were never used.
The REST resource config could still exist in the database or in the config export, which would cause an error stating that the plugins do not exist.

#### What impact will there be?

After running a database update the REST resource config will be removed from the database.
If you have exported the config, it is recommended to remove the REST resource config from your export.
Another guard has been put in place: if the config is included, the import will ignore these specific REST resource configs.

#### What actions do I need to take?

Run a database update and remove the REST resource config from your config export.

#### Are there any risks I should be aware of?

None, this issue should not occur again with both guards in place.

### Page Builder in-place editing for deeply nested components

#### What is it?

Fixed an issue where in-place editing could target the wrong field or show empty values for nested components, especially in deeper component nesting scenarios.

#### What impact will there be?

In-place editing now resolves and opens the correct input/WYSIWYG field in nested component structures, and values are retained consistently when editing.

#### What actions do I need to take?

Clear caches after updating so the nested component templates are rebuilt.

#### Are there any risks I should be aware of?

None.

### Fixes an issue where tokenized values with HTML tags on templates were being rendered literally rather than executed

#### What is it?

Resolves an issue where tokenized values with HTML tags on templates were being rendered literally rather than executed.
This was due to a change in the way the token replacement which caused this regression.

#### What impact will there be?

HTML tags in tokenized values will now be rendered correctly on templates.
Tags that are deemed unsafe will continue to be rendered literally rather than executed, and the rest will be rendered as expected.

#### What actions do I need to take?

Re-save the affected template or run a Site Studio rebuild.

#### Are there any risks I should be aware of?

None.

### Fixes error log flooding when menu templates use tokens alongside menu links from other modules

#### What is it?

Menus that mix a Site Studio menu template with links added by other modules (such as Taxonomy Menu, or menu links generated by Views) were generating an error log entry every time the menu was rendered, if the template used a link token (e.g. link title or URL).

#### What impact will there be?

These sites will no longer see their logs filled with unnecessary error entries, making it easier to spot real issues.
...
### Fixes orphan cleanup stalling when layout revision data is out of sync

#### What is it?

Fixed an issue where the `sitestudio:cleanup-orphans` command could stall without deleting anything if some layout data was left out of sync, for example after a previous cleanup was interrupted.

#### What impact will there be?

Orphan cleanup now completes successfully and removes the leftover data in these cases.
...
Resolves an issue where SCSS variables are not replaced correctly in the generated CSS.
This was a particular issue with colors & values wrapped in SCSS interpolation.

#### What impact will there be?

The SCSS variables are now correctly replaced in the generated CSS.
...
### Fixes an issue where transform values set to zero were removed from the generated CSS

#### What is it?

Resolves an issue where transform values set to zero were removed from the generated CSS.

#### What impact will there be?

The transform values set to zero are now correctly included in the generated CSS.
...
### Fixes an issue when sitestudio_claro is installed on Drupal 11.4

#### What is it?

Resolves a problem where the sitestudio_claro module was not compatible with Drupal 11.4, causing errors during installation or upgrading.

#### What impact will there be?

The sitestudio_claro submodule is now compatible with Drupal 11.4 and checks that the claro.theme file exists before requiring it.

#### What actions do I need to take?

Clear caches (e.g., run `drush cr`) if you are using sitestudio_claro on Drupal 11.4.

#### Are there any risks I should be aware of?

Some styling/layout may be slightly different in the Media Library modal when using the Visual Page Builder.

### Fixes an issue where nested views only rendered the first row

#### What is it?

Resolves a bug where views containing nested views (a view rendered inside another view's row template) would only display the first row of the parent view. This occurred because the view iterator used a single global counter that was reset by nested views, breaking the parent view's iteration.

#### What impact will there be?

Nested views will now render all rows correctly. Each view instance maintains its own iteration state, preventing collisions between parent and child views.

#### What actions do I need to take?

Resave affected template entities or run a rebuild.

### Page Builder no longer reloads after applying unchanged component settings

#### What is it?

Fixed an issue where Page Builder could briefly reload or flicker after clicking Apply on a component when no settings had changed.

#### What impact will there be?

Editors can apply unchanged component settings without the Page Builder canvas unnecessarily refreshing and disrupting their workflow.
...
### Resolving compatibility issues with TMGMT version 1.17+

#### What is it?

Fixed a compatibility issue with TMGMT 1.17+ that prevented newly created translations from being saved.

#### What impact will there be?

The latest versions of TMGMT will now work as expected with Site Studio.
...
The fix is backward compatible and should not cause any issues if you are using an older version of TMGMT.

### Orphaned cohesion layout revision cleanup scalability fix

#### What is it?

Fixes `drush sitestudio:cleanup-orphans` and cron-based cleanup timing out on large sites.

#### What impact will there be?

Sites with large numbers of orphaned revisions can now run the cleanup without timeouts.
...
The cleanup permanently deletes orphaned revision data. Always run with `--dry-run` first.

### Page Builder component styles lost after using CKEditor fullscreen mode

#### What is it?

Fixed an issue where Page Builder components could lose styles after using CKEditor fullscreen mode in Drupal 11.4. This also fixes partial WYSIWYG text being saved when Apply is clicked quickly.

#### What impact will there be?

Component styles will remain visible after using CKEditor fullscreen mode, and WYSIWYG text changes will save correctly.
...
When running a rebuild via Drush, the API keys are now checked to make sure they exist before the rebuild process begins.
This ensures that a rebuild is not run without the necessary API keys, preventing potential errors during the rebuild.

#### What impact will there be?

API keys are now checked when running a rebuild via Drush. If the API keys are not present, the rebuild will not proceed, and an error message will be displayed.
...
The new Drupal token `[media-entity-reference:media-alt]` is available and allows you to fetch the alt text of media entities selected in Image & Picture elements used within components.

#### What impact will there be?

This token will enable site developers to use the existing media's alt text rather than having to input alt text for an Image manually when using media entities in Image & Picture elements.
This will save time and ensure that the alt text is consistent with the media entity's alt text, improving accessibility and SEO.

#### What actions do I need to take?

Navigate to a component that uses an Image or Picture element, and in the alt text field, use the token `[media-entity-reference:media-alt]`.
The token will fetch the alt text from the media entity that is selected in the Image or Picture element.

If a website is using the entity reference or browser elements to select media no action should be required.
...
Adds WAI-ARIA Tabs pattern role and state attributes on accordion tab elements when set to tabs mode to improve accessibility.
This includes `role="tablist"`, `role="presentation"`, `role="tab"`, `role="tabpanel"`, `aria-selected`, `aria-controls`, and `aria-labelledby` attributes. All tab-specific ARIA attributes are removed when the component switches to accordion mode.

#### What impact will there be?

The addition of ARIA role and state attributes will enhance the accessibility of accordion tabs for users relying on assistive technologies, such as screen readers.
It will provide better context, state information, and navigation cues for users interacting with tab components.
...
There is a new system setting which allows users to globally set the Link element to link directly to media files instead of the media entity.
This means that when a user selects a media item in the Link element, they will be linking directly to the file associated with that media item, rather than the media entity itself.

#### What impact will there be?

This change will allow users to easily link to referenced media files where a Link element has been used.

#### What actions do I need to take?

If you want to enable this new behavior, you can go to the system settings and toggle on "Link to media file".
Once enabled, all Link elements that are referencing media entities will link directly to media files instead of the media entities.

#### Are there any risks I should be aware of?

None, this is a global setting that can be easily toggled on or off based on your needs.

### Accordion tabs trigger element can be updated to a button element for improved accessibility

#### What is it?

Users can now set the trigger HTML element for accordion items to use a `<button>` or `<a>` element.

#### What impact will there be?

Using a `<button>` element for accordion triggers improves accessibility by providing better semantics and built-in keyboard support.
The default trigger element remains an `<a>` tag to maintain backward compatibility and styling.

#### What actions do I need to take?

Run a Site Studio import.

Update your accordion items to use the new button trigger option if you want to take advantage of improved accessibility.
This can be configured in the Accordion tabs item element under `Accordion settings` > `Trigger HTML element`.

#### Are there any risks I should be aware of?

The new button trigger option is optional. If you update an existing accordion item from an `<a>` to `<button>`, the trigger styles may need to be adjusted.

### Improved page rendering performance for pages with multiple components

#### What is it?

Performance improvements have been made to reduce redundant processing when rendering pages that contain multiple Site Studio components.
The system now caches and reuses component style calculations and template lookups within a single page request.

#### What impact will there be?

Pages containing many Site Studio components will render faster, particularly those with repeated use of the same components or custom styles.
Sites with complex layouts should see noticeable improvements in page load times.

#### What actions do I need to take?

No action required. The performance improvements are applied automatically.
...
The column-count CSS property was not being applied as expected, which would lead to layout issues when using multi-column layouts.

#### What impact will there be?

This fix ensures that the column-count property is now applied correctly, allowing for proper multi-column layouts as intended.
...
Token replacement values were being incorrectly encoded in templates, resulting in HTML entities being displayed instead of the intended characters.

#### What impact will there be?

This issue could affect any content that uses tokens, such as links or dynamic text, where the replacement value contains special characters.
The values would be displayed as HTML entities (e.g., `&amp;` instead of `&`), which could lead to confusion or incorrect display of content.
...
Resolves a bug where multiple custom components are in a module with the same template filename, causing the wrong template to be used.

#### What impact will there be?

This fix ensures that each custom component correctly uses its own template, even if multiple components share the same template filename.
...
### Fixes an issue where modifer clases were not being applied

#### What is it?

A bug was fixed where modifier classes were not being applied correctly in certain cases, leading to styling issues.

#### What impact will there be?

This fix ensures that modifier classes are now applied correctly, which should resolve any styling issues related to missing classes.
...
Resolves an issue when the content access module is installed and attempting to view layout canvas content an SQL error was occuring.

#### What impact will there be?

The SQL error will no longer occur when using the content access module is installed.
...
Resolves a bug where font libraries containing fonts with numbers in their names (such as "Avenir LT Std 55 Roman") would fail during a rebuild operation.

#### What impact will there be?

Font libraries that previously failed to rebuild due to font naming issues will now rebuild successfully.

#### What actions do I need to take?

No action required. Previously affected font libraries can now be rebuilt.
...
Resolves a bug where pseudo selectors (such as `:nth-child()`, `:hover`, `:focus`) were being incorrectly stripped when used multiple times within a complex CSS selector chain.

#### What impact will there be?

Styles using pseudo selectors in complex selectors will now be generated correctly, ensuring the expected styling is applied.
...
### Updating PHP annotations to PHP attributes

#### What is it?

Updating PHP annotations to PHP attributes in the codebase to align with modern PHP standards and improve code readability and maintainability.
The main bulk of these are internal, with the exception of the CustomElement.
CustomElement now has a PHP attribute for defining metadata instead of using docblock annotations, the annotation can still be used until support is removed in a future release.

It is recommended to update any custom elements to use the new PHP attribute syntax to ensure compatibility with future versions of Site Studio.

#### What impact will there be?

Improved code readability and maintainability with modern PHP attributes, backward compatibility with existing annotations until support is removed in a future release.

#### What actions do I need to take?

It is recommended to update any custom elements in your code base to use the new PHP attribute syntax to ensure compatibility with future versions of Site Studio.
The old annotation syntax will still work for now, but it is best to transition to the new attribute syntax.
...
### Incompatibility with Acquia Web Governance (Optimize) module

#### What is it?

Fixed an issue where Site Studio was incompatible with Acquia Web Governance (Optimize) module, causing JavaScript errors and preventing the WebGov interface from loading correctly.

#### What impact will there be?

Acquia Web Governance (Optimize) scan operations will now work correctly on sites with Site Studio installed, allowing users to access the WebGov interface without errors.
...
### Component canvas preview width option not applying correctly

#### What is it?

Fixes a bug where the component canvas preview width option was not applying correctly, causing the preview to always render at 100% width regardless of the selected option.

#### What impact will there be?

The component canvas preview will now correctly apply the selected width option, allowing for more accurate previews of components.

#### What actions do I need to take?

Run a Drupal cache clear.

#### Are there any risks I should be aware of?

None.

### Pattern repeater item removal leaving blank rows

#### What is it?

Fixed an issue where removing a pattern repeater item containing WYSIWYG fields would leave a blank row instead of properly removing the item.

#### What impact will there be?

Pattern repeater items with WYSIWYG components can now be removed correctly without creating blank rows.

#### What actions do I need to take?

Site Studio import is required to ensure the updated JavaScript assets are loaded.

### Color Palette SCSS variables not rendering correctly

#### What is it?

Fixes an issue when a Color Palette SCSS variable was used in a style, the variable was not being replaced with the correct value and instead the variable name was being output in the generated CSS.

#### What impact will there be?

Color Palette SCSS variables will now be correctly replaced with their corresponding values in the generated CSS.

#### What actions do I need to take?

Run a Site Studio rebuild to regenerate stylesheets with the correct variable replacements.

#### Are there any risks I should be aware of?

None.

### Views exposed filters displayed as blocks were cleared after a page reload

#### What is it?

Fixes an issue where the values of Views exposed filters, displayed as blocks, were cleared after form submission and page reload.

#### What impact will there be?

Views exposed filters will now retain their selected values after submission. The reset button will also work correctly again.

#### What actions do I need to take?

No action required. The fix takes effect immediately after updating.
...
Fixes an issue where element breakpoint CSS `<link>` tags had incorrect `media` attribute values in desktop-first mode. The `media` attribute used the breakpoint's own width instead of the previous (larger) breakpoint's width, causing element styles to load at the wrong viewport widths.

#### What impact will there be?

Element breakpoint CSS `<link>` tags will now have the correct `media` attribute values in desktop-first mode, ensuring element styles load at the intended viewport widths.
...
### "Hide if no data" legacy settings lost after upgrading

#### What is it?

Fixes an issue where the "Hide if no data" settings token on components was lost, causing elements to remain visible even when their associated fields contained no data.

#### What impact will there be?

Components with the legacy "Hide if no data" setting will now be correctly migrated to the new schema and function as expected.
...
### Remote media references breaking page cacheability

#### What is it?

Fixed an issue where pages containing remote media references (such as Bynder) in Site Studio layouts were not being cached by the Dynamic Page Cache, resulting in slower page load times.

#### What impact will there be?

Pages with remote media references in Site Studio layouts will now be correctly cached, improving page performance.
...
### Tokenized styles not rendering on view blocks

#### What is it?

Fixed an issue where tokenized styles from components were not rendering when used inside a view block.

#### What impact will there be?

Tokenized styles from components used in view templates will now render correctly on view blocks.
...
### View full pager not respecting "Number of pager links visible" setting

#### What is it?

Fixed an issue where a Drupal View using the Full pager with "Number of pager links visible" was rendering all page links instead of limiting to the configured quantity.

#### What impact will there be?

The Full pager will now correctly display only the configured number of page links, consistent with Drupal core's pager behavior.
...
Add support for the HTML title attribute on video elements. If the video element uses an iframe, the title will be added to the iframe tag.

#### What impact will there be?

The title attribute will now be included on video elements, providing additional context for accessibility and SEO purposes.

#### What actions do I need to take?

If customers want to utilize this feature, they can add a title to their video elements in the component configuration.
...
### Drupal tokens not being replaced in styles within templates

#### What is it?

Fixes an issue where Drupal tokens were not being replaced in styles that were included within templates, resulting in tokens being output as literal text instead of being replaced with their corresponding values.

#### What impact will there be?

Drupal tokens included in styles within templates will now be correctly replaced with their corresponding values, ensuring that dynamic styling based on tokens works as intended.

#### What actions do I need to take?

Run a Site Studio rebuild or re-save effected templates to regenerate styles.
...
Fixes an issue where the browser automatically scrolled to default-open accordion or tab items on page load, even when no URL hash was present. This occurred whenever the "Scroll to open" toggle was enabled in Accordion or Tabs Container components. Additionally, tabs mode had issues where page reload with a hash would not scroll correctly, and external links to tab items would not scroll on first click.

#### What impact will there be?

Accordion and Tabs Container components will now behave correctly:
- No auto-scroll on page load when URL has no hash
- Proper scroll behavior when hash matches specific accordion/tab item
- Tabs mode scrolls correctly on page reload with hash
- External links to tabs work on first click
- Multiple accordion/tab containers on same page work independently
...
### Element CSS split by breakpoint not ordered correctly for mobile-first grid

#### What is it?

Fixes an issue where element breakpoint CSS files were not ordered correctly when the grid was set to mobile-first, causing incorrect CSS cascade and broken styling (e.g. accordions/tabs).

#### What impact will there be?

Element breakpoint CSS will now be correctly ordered based on the grid type, and responsive grid settings will be saved against the correct theme.
...
### Styles not always being generated in the correct order

#### What is it?

Fixes an issue where styles were not always being generated in the correct order, which could lead to unexpected styling results on the frontend.

#### What impact will there be?

Styles will now be generated in the correct order, ensuring that CSS rules are applied as intended.
...
### Views pagination not rendering after Drupal 11.3 update

#### What is it?

Fixes an issue where views templated in Site Studio and using pagination, the pagination was not rendering after updating to Drupal 11.3 due to updates in pager preprocessing.

#### What impact will there be?

Views pagination will now render correctly in Drupal 10, 11.3 and later versions.
...
### Page Builder textarea fields losing line breaks on edit

#### What is it?

Fixed an issue where textarea fields in Page Builder components lost line breaks when using the "Apply" or "Cancel" buttons in edit-in-place mode. Text content with newlines would either collapse into a single line or disappear entirely.

#### What impact will there be?

Users can now edit textarea fields in Page Builder without losing line breaks. Content with multiple lines will be preserved correctly when clicking "Apply" or "Cancel" during inline editing.
...
### Replacing removed views_views_pre_render function in Drupal 11.3

#### What is it?

Replaces the removed `views_views_pre_render` function that was deprecated in Drupal 11.3 and previously used in the views/infinite scroll integration.

#### What impact will there be?

The views/infinite scroll integration will continue to function correctly in Drupal 11.3 and later versions without relying on the removed function.
...
### Page Builder moderation state transitions issue

#### What is it?

Fixed an issue where Page Builder loaded the default (Published) revision instead of the latest (Draft) revision, causing errors when attempting to transition moderation states from Draft to other states within Page Builder.

#### What impact will there be?

Users can now successfully change moderation states (Draft → In Review → Published) directly from Page Builder without encountering permission errors. The moderation state widget will correctly reflect the current state of the content.

#### What actions do I need to take?

None. The fix will apply automatically after updating to this version.

#### Are there any risks I should be aware of?

None.

### WYSIWYG field repeater causing page to hang

#### What is it?

Fixed an issue where the page would hang when removing a WYSIWYG field from a pattern repeater after clicking the "About text formats" link.

#### What impact will there be?

Users can now safely add, remove, and interact with multiple WYSIWYG fields in pattern repeaters without the page becoming unresponsive.
...
Fixes an issue where accordions set to "scroll to on open" and "create # url", auto-scrolled to the accordion on page load even if there is no hash in the URL.

#### What impact will there be?

On page load it will only scroll to the accordion if there is a hash in the URL.
...
Fixed an issue where blocks placed in Site Studio hidden regions were applying their cache metadata to pages even when access was denied. Also fixed an issue where the `hasDrupalPermission()` Twig function always returned TRUE for single permission strings without checking user permissions.

#### What impact will there be?

Pages with inaccessible blocks will now cache correctly. Only the access result's cache metadata is applied when blocks are not rendered, preventing cache pollution while maintaining proper cache invalidation.
...
### When using CSS transformations the unit 'deg' is always appended to the value

#### What is it?

Fixed an issue when using CSS transforms a suffix of 'deg' was added to the value even if one already existed.
This meant that the CSS value could end up with the unit 'deg' appended twice.

#### What impact will there be?

The CSS value is now checked to see if a unit exists, if not then it will fallback to append 'deg'.

#### What actions do I need to take?

Site Studio rebuild or re-saving of styles where transformations have been used.

#### Are there any risks I should be aware of?

None.

### Font family names with numeric values not loading correctly due to value being stripped

#### What is it?

Fixes an issue where font family names with numeric values were not loaded correctly because the numeric portion was removed, preventing the correct font from being applied.

#### What impact will there be?

Font family names with numeric values will now load and apply correctly.

#### What actions do I need to take?

Site Studio rebuild or re-save font libraries page if a website has numeric values in font family names.
...
Fixes an issue where setting the CSS content property, to a string value of single quotes resulted in the generated output being wrapped in double quotes.

#### What impact will there be?

CSS content property values of single quotes will now retain this as the value, ensuring the value is not wrapped in further quotes.

#### What actions do I need to take?

Site Studio rebuild or re-save styles where the CSS content property uses single quotes string as a value.
...
Fixed an issue where the default text format selected in WYSIWYG field settings was not being applied or persisted correctly.

#### What impact will there be?

The default text format configured in WYSIWYG field settings will now be correctly applied and persisted when reopening settings or after page reload.
...
### Custom styles that were overriding default element styles not applying correctly

#### What is it?

Fixed an issue where custom styles that were overriding default element styles were not applying correctly due to incorrect ordering of the stylesheets.

#### What impact will there be?

Custom styles that override default element styles will now apply correctly, ensuring that intended styles are reflected on the page.
...
### "Hide if no data" setting incorrectly strips HTML embeds from WYSIWYG components

#### What is it?

Fixed an issue where the "Hide if no data" setting was incorrectly stripping HTML embeds (such as iframes, images, and embedded digital assets) from WYSIWYG components when no plain text was present.

#### What impact will there be?

- WYSIWYG components with only HTML embeds (no plain text) will now be correctly recognized as having data and the HTML will no longer be stripped when using the "Hide if no data" setting.

#### What actions do I need to take?

Run a Site Studio rebuild to regenerate templates with the updated "Hide if no data" logic.

#### Are there any risks I should be aware of?

None. The fix only changes how content is detected (checking content length instead of stripping tags), ensuring HTML embeds are preserved while still correctly identifying truly empty fields.

### CKEditor Source view data loss

#### What is it?

Fixed an issue where content entered in CKEditor 5 Source mode could be lost when switching back to visual mode due to a debounce bug in CKEditor's onChange handler.

#### What impact will there be?

Content entered in Source mode will now be preserved when switching between source and visual modes, preventing data loss.
...
Fixed an issue where range sliders were exhibiting flickering behavior when users interacted with the slider in some web browsers. The slider would jump between values unexpectedly, making it difficult to set precise values. This issue was particularly noticeable in gradient percentage sliders and other component settings that made use of range sliders.

#### What impact will there be?

Range sliders will now move smoothly without flickering or jumping between values allowing users to accurately set precise values.
...
### Element default styles not applying correctly

#### What is it?

Fixed an issue where default element CSS that was included in the page, but the media attribute was not always set to the actual media query.
This caused element default styles not to apply correctly and therefore could effect page layouts not rendering as expected.

#### What impact will there be?

The default element styles will now apply correctly based on the active media query.
Site Studio layouts that may not have rendered as expected will now display correctly.

#### What actions do I need to take?

If your website was experiencing layout issues due to element styles not applying correctly, you should run a Site Studio rebuild.
Previously only saving the responsive grid settings would trigger the media queries to update.
...
Fixed an issue where component form submissions would fail with a JavaScript error when using Drupal core's button patch (https://www.drupal.org/project/drupal/issues/1671190). The error occurred because the form submission handler only looked for `<input>` elements with `name="op"`, but the patch converts submit buttons to `<button>` elements. The handler now searches for both `<input>` and `<button>` elements to support both standard Drupal forms and patched forms.

#### What impact will there be?

- Component forms will work correctly with both `<input>` and `<button>` submit elements
- Sites using the Drupal core button patch will no longer experience form submission errors
- No changes to existing functionality for sites using standard input-based submit buttons
...
### Image uploader field overlapping text in background image style

#### What is it?

Fixed an issue where the image uploader field configured as "Background image style" displayed an overlapping figcaption element containing the image filename. This caused text overlap issues in component forms, particularly affecting background and foreground hero components. The figcaption element is now hidden when the image uploader is used as a background image style, while remaining visible in standard file browser mode.

#### What impact will there be?

- Image uploader fields configured as "Background image style" will no longer display the image filename label
- Text will no longer overlap with uploaded images in background image style configurations
- Standard file browser fields will continue to display the image filename as before
- Background image preview maintains proper aspect ratio with improved positioning

#### What actions do I need to take?

None. The fix is automatic and will apply to all image uploader fields configured as background image styles.

#### Are there any risks I should be aware of?

None. This fix only affects the visual display of image uploader fields and does not change any functionality or data handling.

### Keyboard navigation ignores target attributes on menu links

#### What is it?

Resolves an issue where keyboard navigation was not respecting the target attributes (e.g., _blank) on menu links created with Site Studio.

#### What impact will there be?

Keyboard navigation will now correctly respect target attributes on menu links, improving accessibility and user experience.
...
### Large desktop breakpoint icon not rendering correctly in Style Builder

#### What is it?

Fixes a minor UI issue where the large desktop breakpoint icon in the Style Builder did not render correctly.

#### What impact will there be?

The icon will now render correctly in the Style Builder interface.
...
Fixes an issue where tokens inside Site Studio layout canvas components did not resolve values from forward (non-default) revisions. Token replacement now uses the correct parent entity revision that references the current layout canvas revision.

#### What impact will there be?

Draft/forward revisions will correctly display tokenized values from fields updated in those revisions.
Reviewers can see complete content during moderation without publishing.

#### What actions do I need to take?

None. After updating, tokens in draft/forward revisions will render as expected.

### Icon Picker component preview rendering

#### What is it?

Fixes for the Site Studio Icon Picker component to ensure icon previews render correctly with proper font family handling and improved state management.

#### What impact will there be?

- Icon previews now display correctly in both the picker interface and form fields
- Improved state synchronization between icon selection, submission, and reset operations
- Enhanced reliability with proper null/empty checks for icon data
- Added data attributes for better testing and debugging support

#### What actions do I need to take?

- None. The fix is automatically applied and backward compatible.

#### Are there any risks I should be aware of?

- None. Existing icon picker configurations will continue to work as expected.

### Escape HTML toggle not being respected for input fields

#### What is it?

Fixes an issue where the "Escape HTML" toggle on input fields was not being respected.

#### What impact will there be?

Input fields with the "Escape HTML" toggle enabled will now properly escape HTML characters.
...
Site Studio now includes a search functionality in the Style Builder that allows you to quickly find CSS properties by typing in a search bar. The search understands CSS property names, aliases, and technical terms to help you locate the right properties faster.

#### What impact will there be?

- A search input will appear in the Properties menu with placeholder "Search CSS properties..."
- You can search using standard CSS property names (e.g., "font-family", "background-color")
- Search also works with common aliases (e.g., "bg" for background, "w" for width)
- Technical terms like "typography" or "flexbox" will find relevant property groups
- Results show property groups that match your search term
- When no results are found, you'll see "No results - try Custom CSS"

#### What actions do I need to take?

- Use the search bar in the Properties menu to quickly find CSS properties
- Try searching with terms like "color", "spacing", "font", "background", "layout"
- Use the clear button (×) or press Escape to clear your search
- When no matching properties are found, consider using the Custom CSS option
...
- The search feature requires JavaScript to be enabled

### CSS Grid Selectors for Style Builder

#### What is it?

Site Studio now includes CSS Grid support in the Style Builder, allowing you to create grid-based layouts directly through the visual interface. This feature provides:

1. **Grid Container Controls**: Configure grid containers with properties like grid-template-columns, grid-template-rows, grid-gap, and alignment options.

2. **Grid Item Controls**: Position and size grid items using grid-column, grid-row, grid-area, and alignment properties.

3. **Enhanced Validation**: Robust form validation ensures proper grid syntax and prevents invalid configurations.

#### What impact will there be?

- New CSS Grid layout options available in the Style Builder interface
- Existing layouts using other positioning methods will continue to work unchanged

#### What actions do I need to take?

**For Developers**:
- Familiarize yourself with the new CSS Grid options in the Style Builder
- Review existing layouts that might benefit from grid-based approaches
...
- CSS Grid has excellent modern browser support but may require fallbacks for older browsers
- Complex grid layouts should be thoroughly tested across target browsers
...
Fixed an issue where existing field values in field groups were lost when conditional visibility rules were applied to parent containers. This occurred when updating an existing component with new conditional field groups, causing previously saved data in existing field groups to not be retained or displayed when the component form was opened.

The fix ensures that nested conditional fields maintain their values when parent containers are hidden or shown, preventing data loss during component form updates.

#### What impact will there be?

- Field values in conditional field groups will now be properly preserved when parent containers are hidden or shown based on conditional visibility rules
- Existing component data will remain intact when adding new conditional field groups to an existing component
- Nested conditional fields will maintain their values during parent condition changes
- No need to remove conditional visibility as a workaround to retain field data

#### What actions do I need to take?

- None. The fix is automatic and will preserve all field values in components with conditional visibility.
- If you previously removed conditional visibility rules as a workaround, you can now safely re-apply them.
...
Fixes for the Site Studio Components admin list to ensure drag-and-drop and row-weight reordering work reliably in nested accordion structures.
...
Site Studio now provides safe cleanup of orphaned layout canvas revisions through two methods:

1. **Configurable automatic cleanup (disabled by default)**: A cron job can run at configurable intervals to identify and clean up any orphaned layout canvas revisions from any entity type using layout canvas that may exist in the database. Choose from hourly, daily, every 2 days, weekly, or monthly intervals. This feature is off by default and must be enabled in settings.

2. **Manual cleanup command**: A Drush command `sitestudio:cleanup-orphans` is available to manually identify and clean up existing orphaned revisions from any entity type using layout canvas.

#### What impact will there be?

Database table size will be reduced on sites with many orphaned layout canvas revisions. The cohesion_layout_revision table will be cleaned up based on your configured interval or manual drush commands. Performance improvements include better resource management and reduced memory usage during cleanup operations. Content operations like translations and component duplication will not be affected by cleanup processes.

#### What actions do I need to take?

**Initial cleanup (one-time)**:
- Run `drush sitestudio:cleanup-orphans --dry-run` to see how many orphaned revisions exist on your site
- Run `drush sitestudio:cleanup-orphans` to clean them up

**Configuration (optional to enable automatic cleanup)**:
- Go to `/admin/cohesion/settings` and enable "Delete orphaned revisions on cron". Select your preferred interval.

**Ongoing maintenance**:
- Orphans will be cleaned up automatically based on your configured interval, or you can run the drush command manually as needed.

#### Are there any risks I should be aware of?

The cleanup command and cron job permanently delete orphaned revision data. Always run with `--dry-run` first to review what will be deleted.
Consider backing up your database before running the initial cleanup.

### Custom components in-use tracking

#### What is it?

Custom component's usage can now be tracked via the Site Studio in-use system.
Within the components list builder page, next to a custom component it will
now display either an "in-use" link, so users can see where a custom component
has been used or be shown as "Not in use".
...
### Enhanced "hide if no data" feature with backward compatibility

#### What is it?

The "hide if no data" feature has been enhanced to support multiple conditions (OR/AND) and improved UI while maintaining backward compatibility for existing components.

#### What impact will there be?

- Enhanced UI and conditional logic (OR/AND) for hide-no-data element properties with improved form defaults
- Automatic migration of legacy `hideData` configurations to the new `hideDataFields` array structure
- Improved Twig extensions and React hooks to support multiple hide-no-data conditions
- Existing components using the legacy hideData property will continue to work seamlessly

#### What actions do I need to take?

None. The system automatically handles backward compatibility for existing components.

#### Are there any risks I should be aware of?

None. All existing functionality is preserved while new capabilities are added.

### Site Studio API has moved!

#### What is it?

Site Studio's API has moved to a new location - https://api.sitestudio.acquia.com

#### What impact will there be?

A significant re-architecting of Site Studio's style compilation services resulting in significantly improved API response times.

#### What action do I need to take?

Ensure any API URL override via `dx8_editable_api_url` is removed.

#### Are there any risks I should be aware of?

Following upgrade steps verify that all styles have been compiled as expected.

### Expanded conditional field functionality

#### What is it?

Additional operators have been added to the existing condition options for conditional display of form fields.

#### What impact will there be?

A comprehensive set of conditions for evaluating visibility of fields based on complex conditions.
...
A new form building feature that allows you to create conditional logic rules to control field behavior. The decision tree enables you to:
- Show/hide fields based on conditions
- Enable/disable fields dynamically
- Set or clear field values automatically
- Create complex rules with AND/OR logic between conditions

#### What impact will there be?

- New "Decision Tree" tab in field configuration
- Ability to create if-then rules with multiple conditions and actions
- Support for complex expressions like `[field.name] == "value"` or `[field.number] > 100`
- Conditional field interactions without custom code

#### What actions do I need to take?

1. Enable the experimental feature in settings
2. Access the decision tree builder through field configuration
3. Define your conditional rules using the visual interface
4. Test your rules thoroughly in different scenarios

#### Are there any risks I should be aware of?

- This is an experimental feature and may have some limitations
- Complex rule sets might impact form performance
- Always test conditional logic thoroughly before deploying to production
- Consider the order of rule evaluation when multiple rules affect the same field

### User roles can be exported in a sync package

#### What is it?

Drupal user roles can now be included in a Site Studio package export.

#### What impact will there be?

If required Drupal user roles can be included as part of a Site Studio sync package.

#### What actions do I need to take?

Users will need to select user roles to be included in a sync package if required.
User roles can be included/excluded in a full sync package export via the "Full export settings" page (/admin/cohesion/sync/export_settings).
...
Site Studio now allows filtering the component list by dropzone, making it easier to identify which components can benefit from tagging.

#### What impact will there be?

Site builders can now filter components based on whether they are:
- All components
- Components with dropzones
- Components without dropzones

This helps in identifying which components can benefit from tagging and works in combination with existing filters.

#### What actions do I need to take?

No action required. The new filter is available in the component list interface.

#### Are there any risks I should be aware of?

None.

### Event dispatcher added for API errors

#### What is it?

New event added to dispatch debugging data should API requests fail.

#### What impact will there be?

Events are dispatched silently unless a subscriber is configured to collect this data, this feature is primarily for debugging purposes.
...
### Unable to delete users connected to component content

#### What is it?

Addresses an issue where deleting a user and re-assigning component content failed.

#### What impact will there be?

Component content can now be reassigned when deleting users.
...
### Drush 13 compatibility when using --diff option

#### What is it?

Fixes an issue where running `drush sitestudio:package:import --diff` would result in an error when running Drush 13 or higher.

#### What impact will there be?

The `--diff` option can now be used with the `drush sitestudio:package:import` command without compatibility errors.
...
Fixes an issue where multiple Cohesion subthemes caused the WYSIWYG editor to display incorrect button styles.

#### What impact will there be?

Button styles in the editor now always match the current page theme, providing accurate previews.
...
### Installing cohesion and cohesion_templates modules on Drupal 11.2 results in an error

#### What is it?

Fixes an issue where installing cohesion dependencies on Drupal 11.2 resulted in a ServiceNotFoundException error.

#### What impact will there be?

The error will no longer occur when installing all cohesion modules on Drupal 11.2.
...
### Component Tag Entity type needs to be installed error when upgrading

#### What is it?

Addresses an error visible on Drupal status page when upgrading Site Studio to 8.1.0, stating that the `cohesion_component_tag`
entity type needs to be installed.

#### What impact will there be?

Error will no longer be displayed on the Drupal status page.
...
### Strange characters appearing on component content forms

#### What is it?

Fixes an issue where unwanted characters were appearing in component content forms.

#### What impact will there be?

Erroneous characters are no longer rendered on component forms.
...
### Required fields not validated across tabs on component form

#### What is it?

Addresses an issue where required fields were not being validated when tabs were used on component forms.

#### What impact will there be?

Required fields will now be validated across tabs on component forms, ensuring that all required fields are checked
before submission.
...
### Comma separated CSS selectors prefixed with a Combinator returning invalid

#### What is it?

Fixes an issue when a comma separated selector prefixed with a Combinator returns an error from the API

#### What impact will there be?

This will no longer return an error
...
### Modal Popup Click Issue in Carousel Container with Infinite Scrolling Enabled

#### What is it?

Fixes an issue where modal popups were unresponsive when used within a carousel that had infinite scrolling enabled.

#### What impact will there be?

The updated version allows users to seamlessly set up an infinite carousel with optional modal support, ensuring both features work together smoothly.
...
### Files that are no longer in use included in sync packages

#### What is it?

Resolves an issue where files, such as images, that were replaced in a component were still included in the sync export along with the new files.

#### What impact will there be?

Only the files currently in use will be included in the sync export, reducing unnecessary file inclusion.
...
### Resolving removed upload validators for Drupal 11

#### What is it?

Fixes an issue where removed form upload validators were still being used that have been removed in Drupal 11.
This affected the component preview image upload and the import of Site Studio packages in the UI.

#### What impact will there be?

The component preview image upload and the import of Site Studio packages in the UI will now work as expected in Drupal 11.
...
The Visual Page Builder is now compatible with the [Content Lock](https://www.drupal.org/project/content_lock) module (version 3.0 onwards), preventing multiple users from editing the same page simultaneously.
...
To leverage this functionality; version 3.0+ of the [Content Lock](https://www.drupal.org/project/content_lock) module must be installed.
...
If you want to add a `title` attribute to the iframe, you need to enter a title inside the **Video iFrame Title** field in the YouTube embed element settings.
...
In Site Studio 7.5 [advanced select field capabilities](https://sitestudiodocs.acquia.com/7.5/user-guide/advanced-select-field-capabilities) were introduced to provide dynamic sources for select values.
However, if the values were not stored with the component they were being stripped at the point of render.
...
Fixes a bug where the media library modal window was un-styled and displaying behind the sidebar browser when using the visual page builder.
