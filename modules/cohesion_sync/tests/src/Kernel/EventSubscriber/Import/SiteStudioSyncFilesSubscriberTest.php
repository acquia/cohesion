...
class SiteStudioSyncFilesSubscriberTest extends EntityKernelTestBase {
  ...
  /**
   * A colon in the filename must not be parsed as a subdirectory.
   *
   * @covers \Drupal\cohesion_sync\EventSubscriber\Import\SiteStudioSyncFilesSubscriber::handleFileImport
   *
   * @throws \Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException
   ...
   * @throws \Drupal\Core\Entity\EntityStorageException
   */
  public function testHandleFileImportWithColonInFilename() {
    $filename = '1:2-text-over-bg-image.png';

    // Stage the incoming file in a source directory the event points at.
    // Created at runtime so no colon-named fixture is committed to the repo.
    $source_dir = 'public://colon_source';
    $this->fileSystem->prepareDirectory($source_dir, FileSystemInterface::CREATE_DIRECTORY);
    file_put_contents(
      $source_dir . '/' . $filename,
      file_get_contents($this->getPackageFixturePath() . '/image_5.png')
    );

    $files = [
      'file:file:aaaaaaaa-fd41-4f12-bee2-90f6a27219zz' => [
        'fid' => '8',
        'uuid' => 'aaaaaaaa-fd41-4f12-bee2-90f6a27219zz',
        'langcode' => 'en',
        'filename' => $filename,
        'uri' => 'public://cohesion_templates/' . $filename,
        'filemime' => 'image/png',
        'filesize' => '4476',
        'status' => '1',
        'created' => '1634141178',
    ...
    $event = new SiteStudioSyncFilesEvent($files, $source_dir);
    ...
    // The file is written with the colon preserved in the filename, and the
    // "1:" segment is not turned into a directory.
    $this->assertFileExists('public://cohesion_templates/' . $filename);
    $this->assertDirectoryDoesNotExist('public://cohesion_templates/1:');
  }
