...
class ImportFileForm extends FormBase {
   ...
   *   The settings object.
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, PackagerManager $packager_manager, SharedTempStoreFactory $temp_shared_store_factory, UuidInterface $uuid, StorageInterface $config_storage, FileSystemInterface $file_system = NULL, Settings $settings = NULL) {
    ...
    elseif($form_state->getTriggeringElement()['#name'] == 'import_submit'){
      if ($path = $form_state->getValue('import_tarball')) {
        ...
        try {
          ...
          $archiver->extractList($files, $this->settings->get('site_studio_sync', COHESION_SYNC_DEFAULT_DIR), '', FALSE, FALSE);
