...
class CohesionFileStorage extends FileStorage {
  ...
  public function deleteAll($prefix = 'cohesion') {
    ...
    // Delete files listed in FILE_INDEX_FILENAME.
    ...
    // Remove all remaining non-config files (including images, assets, etc.).
    foreach (new \FilesystemIterator($this->directory, \FilesystemIterator::SKIP_DOTS) as $file) {
      if ($file->isFile()) {
        $this->getFileSystem()->delete($file->getPathname());
      }
    }
    ...
    foreach ($file_list as $uuid => $type) {
      if ($type == 'file') {
        ...
        if ($file instanceof FileInterface) {
          ...
          try {
            $this->getFileSystem()->saveData(
              $data,
              $this->directory . self::FILE_METADATA_PREFIX . '.' . $file->uuid() . '.' . parent::getFileExtension(), FileExists::Replace,
            );
          }
          catch (\Exception $e) {
            throw new InvalidDataTypeException($e->getMessage(), $e->getCode(), $e);
          ...
          $this->getFileSystem()->copy($file->getFileUri(), $file_destination, FileExists::Replace);
          $exported_files++;
        }
      }
    }
