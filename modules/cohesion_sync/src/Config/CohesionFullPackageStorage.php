...
class CohesionFullPackageStorage extends CohesionPackageStorageBase implements StorageInterface {
  ...
  public function listAll($prefix = 'cohesion') {
    ...
    // Check enabled entity types has roles.
    $enabled_entities = [];
    if ($this->configManager && $this->configManager->getConfigFactory()) {
      $sync_settings = $this->configManager->getConfigFactory()->get('cohesion.sync.settings');
      $enabled_entities = $sync_settings->get('enabled_entity_types') ?? [];
    }

    // Conditionally add user.role configs utilised by `cohesion_` permissions.
    if (isset($enabled_entities['user_role'])) {
      $user_role_configs = $this->storage->listAll('user.role.');
      // Filter roles based on permissions containing 'cohesion_'.
      $user_role_configs = array_filter($user_role_configs, function ($role_name) {
        $role_config = $this->storage->read($role_name);

        // Admin roles have all permissions implicitly.
        if (!empty($role_config['is_admin'])) {
          return TRUE;
        }

        if (isset($role_config['permissions']) && is_array($role_config['permissions'])) {
          foreach ($role_config['permissions'] as $permission) {
            if (strpos($permission, 'cohesion') !== FALSE) {
              return TRUE;
            }
          }
        }
      ...
      $site_studio_config = array_merge($site_studio_config, $user_role_configs);
    }
