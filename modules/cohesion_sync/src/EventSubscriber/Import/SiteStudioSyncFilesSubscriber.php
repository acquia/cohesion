...
class SiteStudioSyncFilesSubscriber implements EventSubscriberInterface {
  ...
  public function handleFileImport(SiteStudioSyncFilesEvent $event) {
    ...
    foreach ($event->getFiles() as $file) {
      ...
      $destination = $this->fileSystem->dirname($file['uri']);
  ...
  protected function handleExistingFile(FileInterface $existing_file, array $incoming_file) {
    ...
    if ($this->fileHasChanges($existing_file, $incoming_file)) {
      $this->fileSystem->copy($incoming_file_path, $incoming_file['uri'], FileExists::Replace);
   ...
   *
   * Uses the stream-wrapper aware file system service so filenames containing
   * characters such as a colon (e.g. "1:2-text-over-bg-image.png") are kept
   * intact instead of being split into a directory path.
   *
   * @param string $uri
   *   Uri string.
   *
