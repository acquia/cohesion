    ...
    foreach ($in_use_list as $uuid => $type) {
      if ($type === 'cohesion_style_guide_manager') {
        ...
        if ($entity instanceof StyleGuideManager) {
          $updatedCanvas = new LayoutCanvas($source_component_config['json_values']);
          ...
          if (isset($json_values['model'][$target_component_config['uuid']])) {
            foreach ($json_values['model'][$target_component_config['uuid']] as $model_key => $model_value) {
              if (preg_match(ElementModel::MATCH_UUID, $model_key)
                && !in_array($model_key, $component_field_uuids)
              ) {
                if (!isset($broken_entities[$source_component_config['id']])) {
                  $broken_entities[$source_component_config['id']] = [
                    'type' => $source_component_config['type'] ?? $entity->getEntityTypeId(),
                    'label' => $source_component_config['label'],
                    'id' => $source_component_config['id'],
                  ];
                }
                if (!isset($broken_entities[$source_component_config['id']]['affected_entities'][$entity->uuid()])) {
                  $broken_entities[$source_component_config['id']]['affected_entities'][$entity->uuid()] = [
                    'type' => $type,
                    'label' => $entity->label(),
                    'id' => $entity->id(),
                  ];
