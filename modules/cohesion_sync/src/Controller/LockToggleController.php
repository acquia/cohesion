...
class LockToggleController extends ControllerBase {
  ...
  public static function create(ContainerInterface $container) {
    return new static($container->get('cohesion_sync.packager'), $container->get('entity.repository'));
  }
  ...
  public function index(Request $request) {
    ...
    if ($entity = $this->entityRepository->loadEntityByUuid($entity_type_id, $entity_uuid)) {
      ...
      // Show success.
      // @todo
      drupal_set_message($this->t('%template_name has been %status', [
      ...
      drupal_set_message($this->t('Entity could not be loaded.'), 'error');
