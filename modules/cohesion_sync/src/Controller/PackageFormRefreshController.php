...
class PackageFormRefreshController extends ControllerBase {
   ...
   */
  public function __construct(PackagerManager $packagerManager, EntityRepositoryInterface $entityRepository, EntityTypeManagerInterface $entityTypeManager, UsagePluginManager $usagePluginManager) {
    ...
    foreach ($typed_uuids as $type => $uuids) {
      ...
      foreach ($entities as $source_entity) {
        ...
        foreach ($this->packagerManager->buildPackageEntityList([$source_entity], $excluded_entity_type_ids) as $dependency) {
          ...
          $dependency_type = $dependency['type'] ?? NULL;
          if (!is_string($dependency_type) ||
            $dependency_type === '' ||
            !$this->entityTypeManager->hasDefinition($dependency_type)
          ) {
            continue;
          }

          $entity_type_label = ucfirst(
            $this->entityTypeManager
              ->getDefinition($dependency_type)
              ->getPluralLabel()
              ->__toString()
          );

          $group_id = $dependency_type;
