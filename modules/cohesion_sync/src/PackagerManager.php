...
class PackagerManager {
   ...
   */
  public function __construct(EntityRepositoryInterface $entityRepository, EntityTypeManagerInterface $entityTypeManager, SyncPluginManager $sync_plugin_manager, UsageUpdateManager $usage_update_manager, FileSystemInterface $file_system, LoggerChannelFactoryInterface $logger_factory, StorageInterface $config_storage) {
  ...
  public function validatePackageEntry($entry) {
    ...
    if (!is_string($entry_type) || $entry_type === '' || !$this->entityTypeManager->hasDefinition($entry_type)) {
      throw new \Exception("Entity type {$entry_type} not found.");
    }

    // Get the Sync plugin for this entity type.
    try {
      $type_definition = $this->entityTypeManager->getDefinition($entry_type);
  ...
  public function buildPackageStream($entities, $yaml = FALSE, $exclude_entity_type_ids = []) {
    ...
    foreach ($this->buildPackageEntityList($entities, $exclude_entity_type_ids) as $entry) {
      if ($entry) {
        ...
        /** @var SyncPluginInterface $plugin */
        if ($plugin = $this->getPluginInstanceFromType($this->entityTypeManager->getDefinition($entry_type))) {
      ...
      if ($plugin = $this->getPluginInstanceFromType($entity->getEntityType())) {
        ...
        if (!isset($list[$dependency_name]) && !in_array($entity->getEntityTypeId(), $excluded_entity_type_ids)) {
          ...
          foreach ($typed_uuids as $type => $uuids) {
            if (!is_string($type) || $type === '' || !$this->entityTypeManager->hasDefinition($type)) {
              continue;
            }
