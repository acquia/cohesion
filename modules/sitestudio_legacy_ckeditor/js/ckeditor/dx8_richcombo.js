...
CKEDITOR.plugins.add( 'dx8_richcombo', {
  requires: 'floatpanel,listblock,button',

  beforeInit: function( editor ) {
    editor.ui.addHandler( CKEDITOR.UI_DX8_RICHCOMBO, CKEDITOR.ui.dx8_richcombo.handler );
  }
} );
...
( function() {
   ...
   * @member CKEDITOR
   */
  CKEDITOR.UI_DX8_RICHCOMBO = 'dx8_richcombo';

  /**
   * @class
   * @todo
   */
  CKEDITOR.ui.dx8_richcombo = CKEDITOR.tools.createClass( {
    $: function( definition ) {
    ...
    proto: {
      ...
      createPanel: function( editor ) {
        ...
        // http://stackoverflow.com/a/28344281
        ...
        function openElements(els) {
          for (var i = 0; i < els.length; i++) {
            removeClass(els[i], 'closed');
            addClass(els[i], 'open');
          }
        }
        function closeElements(els) {
          for (var i = 0; i < els.length; i++) {
            removeClass(els[i], 'open');
            addClass(els[i], 'closed');
          }
        }
        ...
        panel.onShow = function() {
          ...
          // Get a reference to the iFrame document and add a class to
          // the body.
          var doc = this._.iframe.getFrameDocument().$.documentElement;
          addClass(doc.querySelector('body'), 'dx8-styles-panel');

          // Add a class to the block itself so we can distinguish it
          // from other plugins' panel blocks.
          this.getBlock(panelId).element.addClass('dx8-styles-panel-block');

          // Get the header and group elements.
          var headers = doc.querySelectorAll('.dx8-styles-panel-block h1'),
            groups = doc.querySelectorAll('.dx8-styles-panel-block ul');
          ...
          for (var i = 0; i < headers.length; i++) {
            (function () {
              ...
              // Add basic accordion functionality to the groups.
              if (!hasClass(group, 'dx8-styles-processed')) {
                addClass(group, 'closed');
                addClass(group, 'dx8-styles-processed');
                addClass(header, 'closed');

                header.addEventListener('click', function () {
                  if (hasClass(group, 'closed')) {
                    closeElements(headers);
                    closeElements(groups);
                    openElements([this, group]);
                  }
                  else {
                    closeElements([this, group]);
                  }
                });
              }
            })();
        ...
        panel.onHide = function( preventOnClose ) {
          ...
          var doc = this._.iframe.getFrameDocument().$.documentElement;
          removeClass(doc.querySelector('body'), 'dx8-styles-panel');
    ...
    statics: {
      handler: {
        ...
        create: function( definition ) {
          return new CKEDITOR.ui.dx8_richcombo( definition );
        }
      }
    }
  } );
   ...
   * @todo
   */
  CKEDITOR.ui.prototype.addDx8RichCombo = function( name, definition ) {
    this.add( name, CKEDITOR.UI_DX8_RICHCOMBO, definition );
  };
