(function ($, Drupal, once, drupalSettings) {
  ...
  function contentLocking() {
    ...
    const contentLockData = drupalSettings.cohesion.content_lock;
    const queryParams = new URLSearchParams({
      entity_type: contentLockData.entity_type,
      entity_id: contentLockData.entity_id,
    });

    const modalSettings = {
      url: '/cohesionapi/content-lock?' + queryParams.toString(),
      dialogType: 'modal',
      dialog: {
  ...
  function removeNavigationTopBar() {
  ...
  function enablePageBuilder(ev) {
    ...
    if (drupalSettings.cohesion.content_lock && drupalSettings.user.uid !== drupalSettings.cohesion.content_lock.uid) {
      ...
      if (isDrupalInEditMode()) {
        ...
        // Try Admin Toolbar first, then Navigation.
        const adminToolbarEdit = $("#toolbar-bar .toolbar-icon-edit");
        if (adminToolbarEdit.length) {
          adminToolbarEdit.click();
        } else {
          const navEditButton = $(".top-bar .toolbar-button--icon--pencil[href*='/edit']");
          if (navEditButton.length) {
            navEditButton.click();
          }
        }
      }
      ...
      // Capture toolbar height before hiding so React can use it.
      // Works for both Admin Toolbar and Navigation top bar.
      const toolbarBar = document.getElementById("toolbar-bar");
      const topBar = document.querySelector(".top-bar");
      drupalSettings.cohesion.toolbarHeight = toolbarBar
