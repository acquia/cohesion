...
class SitestudioPageBuilderController extends ControllerBase {
   ...
   *
   */
  public function saveFrontendBuilder(Request $request): CohesionJsonResponse {
    ...
    foreach ($data->canvases as $canvas_name => $canvas_data) {
      ...
      if ($entity == NULL) {
        ...
        // Fix: Load latest revision instead of default for moderated content
  ...
  public function buildLayoutCanvas(Request $request) {
    $canvas_id = $request->request->get('canvas_id') ?? $request->query->get('canvas_id');
    if ($canvas_id) {
      $layoutId = explode('-', $canvas_id);
      if (isset($layoutId[1])) {
        ...
        if ($layout = CohesionLayout::load($layoutId[1])) {
          ...
          // Should a content lock be set?
          if (sitestudio_page_builder_is_content_lock_enabled($layout->getParentEntity()->getEntityTypeId())) {
            $lockService = \Drupal::service('content_lock');
            $currentLang = $this->languageManager()->getCurrentLanguage(LanguageInterface::TYPE_CONTENT)->getId();
            $entity = $layout->getParentEntity();
            $lock = $lockService->fetchLock($entity, $currentLang);
            // If there is no lock then lock the entity.
            if (!$lock) {
              $lockService->locking($entity, '*', $this->user->id(), TRUE);
            }
          }
   ...
   * @return array[]
   */
  public function contentLock(Request $request): array {
    $contentLockData = [
      'entity_type' => $request->query->get('entity_type'),
      'entity_id' => $request->query->get('entity_id'),
    ];

    // Check if content lock is enabled for this entity type
    if (!sitestudio_page_builder_is_content_lock_enabled($contentLockData['entity_type'])) {
      return [
        'message' => [
          '#markup' => t('Content lock is not enabled for this entity type.'),
        ],
      ];
    }
    ...
    $lockService = \Drupal::service('content_lock');
    $currentLang = $this->languageManager()->getCurrentLanguage(LanguageInterface::TYPE_CONTENT)->getId();
    $entity = $this->entityTypeManager()->getStorage('node')->load($contentLockData['entity_id']);
    $lock = $lockService->fetchLock($entity, $currentLang);
    $translation_lock = $lockService->isTranslationLockEnabled($contentLockData['entity_type']);
    $unlock = [];
    if ($this->currentUser()->hasPermission('break content lock')) {
      $unlock = $lockService->unlockButton($entity, '*', \Drupal::request()->query->get('destination'));
      $unlock['#title'] = t('Break the lock.');
      $unlock['#prefix'] = '<div class="content-lock-unlock">';
      $unlock['#suffix'] = '</div>';
    }

    return [
      'message' => [
        '#markup' => $lockService->displayLockOwner($lock, $translation_lock) , $unlock,
      ],
    ];
   ...
   * @return array[]
   */
  public function releaseLock(int $node_id): array {
    if (sitestudio_page_builder_is_content_lock_enabled('node')) {
      $lockService = \Drupal::service('content_lock');
      $currentLang = $this->languageManager()->getCurrentLanguage(LanguageInterface::TYPE_CONTENT)->getId();
      $entity = $this->entityTypeManager()->getStorage('node')->load($node_id);

      if ($entity instanceof EntityInterface) {
        $lock = $lockService->fetchLock($entity, $currentLang);
        if ($lock) {
          $lockService->release($entity, '*', $this->user->id());
        }
      }
    }
