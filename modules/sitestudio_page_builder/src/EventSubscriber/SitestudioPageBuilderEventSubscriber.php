...
class SitestudioPageBuilderEventSubscriber implements EventSubscriberInterface {
  ...
  public function registerFrontendUrls(CohesionJsAppUrlsEvent $event) {
    ...
    $lib = $this->libraryDiscovery->getLibraryByName('cohesion', 'cohesion-frontend-edit-scripts');
  ...
  public function alterViewBuilder(CohesionLayoutViewBuilderEvent $event) {
    ...
    if ($route_entity && $host && $host->getEntityTypeId() == $route_entity->getEntityTypeId() && $host->id() == $route_entity->id() && $host->access('update')) {
      ...
      $transition_labels = [];
      $content_moderation_installed = $this->moduleHandler->moduleExists('content_moderation');
      if ($content_moderation_installed && $this->moderationInformation && $this->moderationInformation->isModeratedEntity($latest_host)) {
        // Get the states the entity can transition into and the current state.
        $current_state = $this->moderationInformation->getOriginalState($latest_host);
        $build['#attached']['drupalSettings']['cohesion']['currentState']['label'] = $current_state->label();
        $build['#attached']['drupalSettings']['cohesion']['currentState']['state'] = $current_state->id();

        if ($transitions = $this->stateTransitionValidation->getValidTransitions($latest_host, $this->user)) {
          foreach ($transitions as $transition) {
            ...
            if ($current_state->id() === $transition_to_state->id()) {
   ...
   *   The moderation information service.
   * @param \Drupal\content_moderation\StateTransitionValidation $state_transition_validation
   *   The state transition validation service.
   */
  public function setModerationInformation(
