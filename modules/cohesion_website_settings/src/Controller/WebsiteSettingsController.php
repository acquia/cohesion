...
class WebsiteSettingsController extends ControllerBase implements ContainerInjectionInterface {
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
          ...
          if (in_array($ext, $accepted_extensions)) {
            ...
            try {
              ...
              // Fix: unset by value, not by key.
              if (($key = array_search($ext, $accepted_extensions)) !== FALSE) {
                unset($accepted_extensions[$key]);
              }
            }
            catch (\Throwable $e) {
            }
  ...
  public function elementAction(Request $request) {
    $group = ($request->attributes->get('group')) ? $request->attributes->get('group') : NULL;
  ...
  /**
   * Build batch operations for rebuilding Site Studio entities.
   *
   * @param bool $cron
   *   Whether this is a cron run.
   * @param bool $verbose
   *   Whether to output verbose messages.
   * @param bool $no_cache_clear
   *   Whether to skip cache clearing.
   * @param bool $syncRebuild
   *   Whether this is a sync rebuild.
   *
   * @return array|\Symfony\Component\HttpFoundation\RedirectResponse|null
   *   The batch array or a redirect response.
   *
   * @throws \Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public static function batch($cron = FALSE, $verbose = FALSE, $no_cache_clear = FALSE, $syncRebuild = FALSE) {

    if ($syncRebuild) {
      $syncRebuild = &drupal_static('site_studio_sync_rebuild');
      $syncRebuild = TRUE;
    }
    ...
    // Get config types and filter out ones that don't start with cohesion_.
    $configs = array_filter(
      $entity_type_manager->getDefinitions(),
      fn($entity_type_id) => str_starts_with($entity_type_id, 'cohesion_'),
    ...
    // Config types that are categories, processed but excluded from the
    // "no update" bulk save pass below.
    ...
    // Get media queries from cohesion.settings.
    $media_queries = \Drupal::service('config.factory')->get('cohesion.settings')->get('media_queries');
    $responsive_grid_needs_update = empty($media_queries) || $media_queries === '{}';
    ...
    foreach ($style_configs as $style_config_type) {
      if (isset($configs[$style_config_type])) {
        $storage = $entity_type_manager->getStorage($style_config_type);
        // Get entity ids needing an Site Studio update.
        $entity_ids_needs_update = $storage->getQuery()
          ->accessCheck(FALSE)
          ->condition('status', TRUE)
          ->condition('last_entity_update', $entity_update_manager->getLastPluginId(), '<>')
          ->execute();

        // Add the responsive grid settings to needs update list
        // if cohesion.settings media_queries is empty, even if there are
        // no outstanding entity updates.
        if ($style_config_type === 'cohesion_website_settings' && $responsive_grid_needs_update) {
          // Check the responsive grid settings isn't already in the list.
          if (!in_array('responsive_grid_settings', $entity_ids_needs_update, TRUE)) {
            $entity_ids_needs_update[] = 'responsive_grid_settings';
          }
        }

        for ($i = 0; $i < count($entity_ids_needs_update); $i += $entity_to_process) {
          $ids = array_slice($entity_ids_needs_update, $i, $entity_to_process);
          $batch['operations'][] = [
        ...
        if (in_array($style_config_type, $allowed_config_types)) {
          unset($configs[$style_config_type]);
          continue;
        }

        $entity_ids_no_update = $storage->getQuery()
          ->accessCheck(FALSE)
          ->condition('status', TRUE)
          ->condition('id', $entity_ids_needs_update, 'NOT IN')
          ->execute();

        for ($i = 0; $i < count($entity_ids_no_update); $i += $entity_to_process) {
          $ids = array_slice($entity_ids_no_update, $i, $entity_to_process);
