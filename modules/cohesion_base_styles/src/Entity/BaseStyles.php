...
use Drupal\cohesion_base_styles\BaseStylesListBuilder;
use Drupal\cohesion_base_styles\Form\BaseStylesDeleteForm;
use Drupal\cohesion_base_styles\Form\BaseStylesDisableForm;
use Drupal\cohesion_base_styles\Form\BaseStylesEnableForm;
use Drupal\cohesion_base_styles\Form\BaseStylesForm;
...
#[ConfigEntityType(
  id: 'cohesion_base_styles',
  label: new TranslatableMarkup('Base style'),
  label_collection: new TranslatableMarkup('Base styles'),
  label_singular: new TranslatableMarkup('Base style'),
  label_plural: new TranslatableMarkup('Base styles'),
  config_prefix: 'cohesion_base_styles',
  entity_keys: [
    'id' => 'id',
    'label' => 'label',
  ...
  handlers: [
    'list_builder' => BaseStylesListBuilder::class,
    'form' => [
      'add' => BaseStylesForm::class,
      'edit' => BaseStylesForm::class,
      'duplicate' => BaseStylesForm::class,
      'delete' => BaseStylesDeleteForm::class,
      'enable' => BaseStylesEnableForm::class,
      'disable' => BaseStylesDisableForm::class,
    ],
    'route_provider' => [
  ...
  links: [
    'canonical' => '/admin/cohesion/styles/cohesion_base_styles/{cohesion_base_styles}/edit',
    'edit-form' => '/admin/cohesion/styles/cohesion_base_styles/{cohesion_base_styles}/edit',
    'add-form' => '/admin/cohesion/styles/cohesion_base_styles/add',
    'delete-form' => '/admin/cohesion/styles/cohesion_base_styles/{cohesion_base_styles}/delete',
    'duplicate-form' => '/admin/cohesion/styles/cohesion_base_styles/{cohesion_base_styles}/duplicate',
    'collection' => '/admin/cohesion/styles/cohesion_base_styles',
    'disable' => '/admin/cohesion/styles/cohesion_base_styles/{cohesion_base_styles}/disable',
    'enable' => '/admin/cohesion/styles/cohesion_base_styles/{cohesion_base_styles}/enable',
  ],
  admin_permission: 'administer base styles',
  label_count: [
    'singular' => '@count base style',
...
class BaseStyles extends CohesionConfigEntityBase implements CohesionSettingsInterface {
  ...
  /**
   * Reorders the BaseStyles so that keys not prefixed with 'base_' (i.e.
   * default base styles) come first, followed by user created base styles
   * prefixed with 'base_'.
   *
   * @param array $styles
   *   The array of styles to reorder.
   *
   * @return array
   *   The reordered array with default styles (non-'base_') keys first and
   *   user-created ('base_'-prefixed) keys afterward.
   */
  public static function reorderStyles(array $styles): array {
    $non_base = [];
    $base = [];
    // First, sort the styles by key to ensure consistent ordering.
    ksort($styles);

    foreach ($styles as $key => $value) {
      if (str_starts_with($key, self::ENTITY_MACHINE_NAME_PREFIX)) {
        $base[$key] = $value;
      }
      else {
        $non_base[$key] = $value;
    ...
    // Reverse both arrays to maintain the order before batching was
    // introduced while ensuring non-base styles come first.
    return array_reverse($non_base, TRUE) + array_reverse($base, TRUE);
  }
