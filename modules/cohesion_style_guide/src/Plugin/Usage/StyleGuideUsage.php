...
#[Usage(
  ...
  name: new TranslatableMarkup("Style guide usage"),
...
class StyleGuideUsage extends UsagePluginBase {
  ...
  // Use a regex to scan the JSON blob for usages for style guides.
  const STYLE_GUIDE_MATCH_REGEX = '/\[style-guide:(.*?):(.*?)\]/m';
  ...
  public function scanForInstancesOfThisType($data, EntityInterface $entity) {
    ...
    // Get all the style guides used.
    foreach ($data as $entry) {
      // Search cohesion_layout canvases and potentially WYSIWYG content.
      if (in_array($entry['type'], ['json_string', 'string'])) {
        // Get all the style guide used by style-guide token.
        preg_match_all(self::STYLE_GUIDE_MATCH_REGEX, $entry['value'], $matches, PREG_SET_ORDER, 0);
