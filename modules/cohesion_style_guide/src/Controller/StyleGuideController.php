...
class StyleGuideController extends ControllerBase {
  ...
  public function buildStyleGuidePreview(Request $request) {
    $json_values = $request->getContent();
    ...
    $preview_sgm_values = $json_values;
    ...
    if(property_exists($decoded_json, 'model') && property_exists($decoded_json, 'changedFields')) {
      foreach ($decoded_json->model as $style_guide_uuid => $style_guide_values) {
        ...
        foreach ($style_guide_values as $style_guide_value_uuid => $style_guide_value) {
          if(in_array('model.' . $style_guide_uuid . '.' . $style_guide_value_uuid, $decoded_json->changedFields)
