 ...
 */
class TwigExtension extends AbstractExtension implements TrustedCallbackInterface {
  ...
  /**
   * Per-view row iteration counters, keyed by ViewExecutable instance.
   *
   * Stored on the object (not in Twig $context), since Twig copies
   * $context when entering a {% block %} (used by component drop zones).
   *
   * Keys are spl_object_hash()es, so they don't survive serialization;
   * only safe as long as this stays request-scoped (not cached/lazy-static).
   *
   * @var array<string,int>|null
   */
  protected ?array $viewIterateCounters = NULL;
  ...
  public function __construct(
    ...
    try {
      ...
      // Custom components service is optional; silently fail if not available.
    }
  }
  ...
  public function getFunctions() {
    return [
      ...
      new TwigFunction('base_path', [$this, 'getBaseUrlPath']),
    ];
  }
  ...
  public function tokenReplace($token, $data = NULL, $_context = [], $isTemplate = FALSE) {
    ...
    // If this specific token is used we need to replace it with the media or
    // DAM asset ALT.
    if ($token === '[media-entity-reference:media-alt]') {
      return $this->handleMediaAltToken($_context);
    }
    ...
    if (array_key_exists('menu_link_content', $data) || array_key_exists('menu-link', $data)) {
      ...
      elseif (isset($_context['item']['original_link'])) {
        ...
        // Load the menu link content as an entity. Non-entity-backed menu
        // links (e.g. from taxonomy_menu or Views) will not have a valid
        // entity type as the plugin ID prefix, so guard against those
        // before attempting to resolve the entity.
        try {
          if (isset($def[0]) && isset($def[1]) && $this->entityTypeManager->hasDefinition($def[0])) {
    ...
    $token_replacement = $this->token->replace($token, $data, [
      'langcode' => $language,
      'clear' => TRUE,
    ]);
    ...
    // Encode any HTML capable of executing JavaScript so it displays as
    // literal text rather than executing, while leaving other markup (e.g.
    // wysiwyg content) to render normally.
    if ($isTemplate) {
      $token_replacement = $this->cohesionUtils->escapeExecutableHtml($token_replacement);
    }
  ...
  /**
   * Handles the special media-alt token replacement.
   *
   * @param array $context
   *   The Twig context.
   *
   * @return string|null
   *   The media alt text if found, NULL otherwise.
   */
  private function handleMediaAltToken(array $context): ?string {
    $layout_canvas = ($context['layout_builder_entity']['entity'] ?? NULL)?->getLayoutCanvasInstance();
    if (!isset($context['componentUuid']) || !$layout_canvas) {
      return NULL;
    }

    $isRepeater = isset($context['coh_repeater_val']) || (isset($context['_seq']) && isset($context['_key']));

    foreach ($layout_canvas->iterateCanvas() as $element) {
      if (!$element->isComponent() || $context['componentUuid'] !== $element->getUUID()) {
        continue;
      }

      $result = $isRepeater
        ? $this->findMediaAltInRepeater($element, $context)
        : $this->findMediaAltInElement($element);
  ...
  /**
   * Finds media alt text within a repeater context.
   *
   * @param mixed $element
   *   The canvas element.
   * @param array $context
   ...
   *   The media alt text if found, NULL otherwise.
   */
  private function findMediaAltInRepeater($element, array $context): ?string {
    $indices = $this->buildRepeaterIndexPath($context);
    foreach ($element->getModel()->getValues() as $element_model_values) {
      if (is_array($element_model_values)) {
        $result = $this->findMediaReferenceByIndexPath($element_model_values, $indices);
        if ($result !== NULL) {
          return $result;
        }
      }
  ...
  /**
   * Finds media alt text within a non-repeater element.
   *
   * @param mixed $element
   *   The canvas element.
   *
   ...
   *   The media alt text if found, NULL otherwise.
   */
  private function findMediaAltInElement($element): ?string {
    foreach ($element->getModel()->getValues() as $element_model_value) {
      if (is_string($element_model_value) && str_contains($element_model_value, 'media-reference:media')) {
        return $this->fetchMediaAlt($element_model_value);
      }
    }
    return NULL;
  }
  ...
  /**
   * Builds the full index path for nested repeaters by traversing
   * _parent contexts.
   *
   * @param array $context
   ...
   *
   * @return array
   *   Array of indices from outermost to innermost repeater.
   */
  private function buildRepeaterIndexPath(array $context): array {
    $indices = [];
    $current = $context;

    // Traverse up through parent contexts collecting all _key values.
    while (isset($current['_key'])) {
      $index = (int) str_replace('#', '', $current['_key']);
      array_unshift($indices, $index);
      // Move to parent context if it exists.
  ...
  /**
   * Finds media reference by following an index path through nested structures.
   *
   * @param mixed $structure
   *   The data structure to search.
   * @param array $indices
   *   Array of indices representing the path through nested repeaters.
   *
   * @return string|null
   *   The media alt text if found, NULL otherwise.
   */
  private function findMediaReferenceByIndexPath(mixed $structure, array $indices): ?string {
    if (empty($indices)) {
      return $this->searchForMediaReference($structure);
    }

    $currentIndex = array_shift($indices);
    $iterable = is_object($structure) ? get_object_vars($structure) : (is_array($structure) ? $structure : []);
    $result = NULL;

    // Check if structure is directly indexable at this level first.
    if (is_array($structure) && isset($structure[$currentIndex])) {
      $result = $this->findMediaReferenceByIndexPath($structure[$currentIndex], $indices);
    }

    // Search through object properties for indexed arrays.
    foreach ($iterable as $value) {
      if ($result !== NULL) {
        break;
      }
      $result = match (TRUE) {
        is_array($value) && isset($value[$currentIndex]) => $this->findMediaReferenceByIndexPath($value[$currentIndex], $indices),
        is_object($value) => $this->findMediaReferenceByIndexPath($value, array_merge([$currentIndex], $indices)),
        default => NULL,
      };
    }
  ...
  /**
   * Recursively searches a structure for media references without index
   * constraints.
   *
   * @param mixed $structure
   ...
   *   The media alt text if found, NULL otherwise.
   */
  private function searchForMediaReference(mixed $structure): ?string {
    if (is_string($structure)) {
      return str_contains($structure, 'media-reference:media') ? $this->fetchMediaAlt($structure) : NULL;
    }
    ...
    foreach ($iterable as $value) {
      $result = match (TRUE) {
        is_string($value) && str_contains($value, 'media-reference:media') => $this->fetchMediaAlt($value),
        is_object($value) || is_array($value) => $this->searchForMediaReference($value),
        default => NULL,
      };
      if ($result !== NULL) {
        break;
  ...
  /**
   * Fetches the alt text from a media or media DAM entity.
   *
   * @param string $element_model_value
   *   The media reference string (e.g., "[media-reference:media:uuid]").
   *
   * @return string
   *   The alt text, or empty string if not found.
   */
  private function fetchMediaAlt(string $element_model_value): string {
    $elementModel = explode(':', str_replace(']', '', $element_model_value));
    $media = (isset($elementModel[1], $elementModel[2]) && $elementModel[1] === 'media')
      ? $this->entityTypeManager->getStorage('media')->loadByProperties(['uuid' => $elementModel[2]])
      : [];
    $media = reset($media) ?: NULL;

    if (!$media) {
      return '';
    }

    // If the media is an Acquia DAM asset.
    if (class_exists('\Drupal\acquia_dam\Entity\ImageAltTextField')) {
      $altFieldName = constant('\Drupal\acquia_dam\Entity\ImageAltTextField::IMAGE_ALT_TEXT_FIELD_NAME');
      if ($media->hasField($altFieldName)) {
        return $media->get($altFieldName)->value ?? '';
      }
    }

    // Standard media entity - find field with alt text.
    $altFields = array_filter(
      $media->getFields(),
      fn($field, $key) => $key !== 'thumbnail' && !empty($field->getFieldDefinition()->getSettings()['alt_field']),
      ARRAY_FILTER_USE_BOTH
    );
    $altFieldKey = array_key_first($altFields);

    return $altFieldKey ? ($media->get($altFieldKey)->alt ?? '') : '';
  }
  ...
  /**
   * Build a stable component UUID path for nested component rendering.
   *
   * @param array $_context
   * @param string|null $componentInstanceUuid
   *
   * @return array
   */
  private function build_component_uuid_path(array $_context, $componentInstanceUuid = NULL) {
    $path = [];
    if (isset($_context['componentUuidPath']) && is_array($_context['componentUuidPath'])) {
      foreach ($_context['componentUuidPath'] as $path_item) {
        if (is_string($path_item) && trim($path_item) !== '') {
          $path[] = $path_item;
        }
      }
    }

    if (is_string($componentInstanceUuid) && trim($componentInstanceUuid) !== '' &&
        (empty($path) || end($path) !== $componentInstanceUuid)) {
      $path[] = $componentInstanceUuid;
    }
  ...
  public function renderComponent($componentId, $isTemplate, $_context, $componentFields, $componentInstanceUuid = NULL, $componentContentId = FALSE) {
    ...
    if ($component) {
      ...
      $component_uuid_path = $this->build_component_uuid_path($_context, $componentInstanceUuid);
      ...
      $renderer = [
        ...
        '#componentUuidPath' => $component_uuid_path,
        '#template' => $template,
      ];
    ...
    // If the element is not a loadable component, it might be a custom
    // component.
      ...
      try {
        ...
        if (isset($custom_components[$componentId])) {
          ...
          if (!is_null($custom_components[$componentId]['template'])) {
            ...
            $renderer['#theme'] = 'custom_component_' . str_replace('-', '_', $componentId);
  ...
  /**
   * Renders a Drupal block.
   *
   * @param string|\Twig\Markup $id
   *   The block ID.
   *
   * @return array
  ...
  public function drupalBlock($id) {
    ...
    if ($block instanceof Block) {
      ...
      if ($access->isAllowed()) {
        if ($block->getRegion() === 'dx8_hidden') {
          // Render in isolation to prevent cache metadata leakage.
          $build = [
            '#lazy_builder' => [static::class . '::lazyBuildHiddenBlock', [$block->id()]],
            '#create_placeholder' => TRUE,
          ];
          CacheableMetadata::createFromObject($access)->applyTo($build);
        }
        else {
          $block_build = $this->entityTypeManager->getViewBuilder('block')->view($block);
          $build = $block_build;
  ...
  /**
   * #lazy_builder callback: renders a dx8_hidden block in isolation.
   */
  public static function lazyBuildHiddenBlock(string $block_id): array {
    ...
    if (method_exists($renderer, 'renderInIsolation')) {
      $html = $renderer->renderInIsolation($block_build);
    }
    else {
      $html = $renderer->renderPlain($block_build);
    }
    // Capture #attached and #cache after render so they bubble up correctly.
    // This array is the #lazy_builder placeholder replacement, so it is the
    // ONLY #cache core sees for this block. Dropping contexts/max-age here
    // (as a prior revision of this method did) causes rendered output that
    // actually varies by session/user, or that is genuinely uncacheable, to
    // be stored in the page/CDN cache as if it were static markup — e.g. a
    // CSRF token or user-specific Views output from one request served back
    // to a different user. Bubbling the block's real #cache in full keeps
    // that variation intact: a block with no such dependency is unaffected,
    // and only blocks that truly require it make their containing page less
    // cacheable, which is the correct tradeoff (a wrongly-shared cache entry
    // is a security issue; an uncacheable page is only a performance cost).
    $attached = $block_build['#attached'] ?? [];
    $cache = $block_build['#cache'] ?? [];
    return [
      '#type' => 'markup',
  ...
  public function drupalViewItem($view_modes, $rows) {
    // Support both: a full view render array with '#rows' and a single
    // row render array (templates often pass rows[getViewIterate()]).
    if (is_array($rows) && isset($rows['#rows']) && is_array($rows['#rows'])) {
      // Full view render array provided: keep original behaviour and pull
      // the first element off the '#rows' array.
      $resultrow = array_shift($rows['#rows']);
    }
    else {
      // Single row render array passed directly from template.
      $resultrow = $rows;
    }
    ...
    // If a view mode is present or can be applied, ensure view mode and
    // cache keys are set.
    if (is_array($resultrow) && isset($resultrow['#view_mode'])) {
      ...
      if (is_array($view_modes)) {
        ...
        foreach ($view_modes as $view_mode_data) {
          ...
          if ($entity_type && ($view_mode_data['entity_type'] == $entity_type)
            && $view_mode_data['view_mode'] !== 'default') {
        ...
        if ($entity instanceof ContentEntityInterface && isset($view_modes_bundle[$entity->bundle()])) {
      ...
      if (!isset($resultrow['#cache']) || !is_array($resultrow['#cache'])) {
        $resultrow['#cache'] = [];
      }

      if (!isset($resultrow['#cache']['keys']) || !is_array($resultrow['#cache']['keys'])) {
        $resultrow['#cache']['keys'] = [];
      }
  ...
  public function cohesionViewsPagination($view, $view_data) {
    ...
    if ($view->display_handler->renderPager() && isset($view_data['settings']['pager']['view_pager']) && property_exists($view, 'pager')) {
      ...
      // Invoke initial preprocess (Drupal 11.3+) before preprocess functions.
      ...
      if (!empty($entity_type) && !empty($view_mode) && !empty($entity_id)) {
        ...
        if ($entity instanceof EntityInterface && $entity->access('view', $this->currentUser)) {
          ...
          if ($access->isAllowed()) {
            ...
            // Prevent cache pollution when access denied.
            $build = [];
          }

          // Merge access cacheability without overwriting cache contexts.
          CacheableMetadata::createFromRenderArray($build)
  ...
  public function getComponentFieldValue($context, $key, $sub_key_path = NULL) {
    $keys = [];
    // If we need to find further down the array.
  ...
  public function setViewIterate($context) {
    $this->viewIterateCounters ??= [];
    $this->viewIterateCounters[$this->getViewIterateKey($context)] = 0;
    // Return empty string to avoid printing from templates that echo this call.
    return '';
  }
  ...
  public function getViewIterate($context) {
    return $this->viewIterateCounters[$this->getViewIterateKey($context)] ?? 0;
  }
  ...
  public function incrementViewIterate($context) {
    $this->viewIterateCounters ??= [];
    $key = $this->getViewIterateKey($context);
    $this->viewIterateCounters[$key] = ($this->viewIterateCounters[$key] ?? 0) + 1;
    // Return empty string so templates that echo this function don't output a
    // number.
    return '';
  }

  /**
   * Generate a key identifying the view in scope for iterator storage.
   *
   * @param array $context
   *   The Twig context passed to the Twig function.
   *
   * @return string
   *   A key unique to the view object, or a default if none is present.
   */
  private function getViewIterateKey(array $context): string {
    // Accepts any object by convention, not instanceof ViewExecutable,
    // since tests/other callers may pass a plain stdClass. Keep private.
    $view = $context['view'] ?? NULL;
    return is_object($view)
      ? 'v_' . spl_object_hash($view)
      : 'coh_view_default';
  }

  /**
   * Render field UUID for in-place editing.
   *
   * @param array $context
   * @param string $value
   *
  ...
  public function renderFieldUuid($context, $value) {
    ...
    $component_uuid_path = $this->extract_component_uuid_path($context);
    ...
    return $this->process_layout_canvas_elements(
      $layout_canvas,
      $instance_uuid,
      $value,
      $context,
  ...
  /**
   * Extract component UUID path from context.
   *
   * @param array $context
   *
   * @return array
   */
  private function extract_component_uuid_path(array $context) {
    $component_uuid_path = [];
    if (isset($context['componentUuidPath']) && is_array($context['componentUuidPath'])) {
      foreach ($context['componentUuidPath'] as $path_item) {
        if (is_string($path_item) && trim($path_item) !== '') {
          $component_uuid_path[] = $path_item;
        }
      }
    }
    return $component_uuid_path;
  }

  /**
   * Process layout canvas elements to find matching component.
   *
   * @param object $layout_canvas
   * @param string $instance_uuid
   * @param string $value
   * @param array $context
   * @param array $component_uuid_path
   *
   * @return string
   */
  private function process_layout_canvas_elements(
    $layout_canvas,
    $instance_uuid,
    $value,
    $context,
    ...
    foreach ($layout_canvas->iterateCanvas() as $element) {
      ...
      // Handle pattern repeater case
      if (isset($context['coh_repeater_val']) || (isset($context['_seq']) && isset($context['_key']))) {
        return $this->handle_repeater_case($element, $value, $context, $component_uuid_path);
      }

      return $this->fetch_nested_component_field_uuid($element, $value, NULL, NULL, $component_uuid_path);
    }
  ...
  /**
   * Handle repeater case for field UUID resolution.
   *
   ...
   * @return string
   */
  private function handle_repeater_case($element, $value, $context, $component_uuid_path) {
    foreach ($element->getModel()->getValues() as $repeater_key => $element_model_values) {
      ...
      return $this->fetch_nested_component_field_uuid($element, $value, $repeater_key, $index, $component_uuid_path);
    }
   ...
   * @param object $element
   * @param string $value
   * @param string|null $repeater_key
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  private function fetch_nested_component_field_uuid(
    $element,
    $value,
    $repeater_key = NULL,
    $index = NULL,
    array $component_uuid_path = [],
  ) {
    $result = $value;
    $component = $this->entityTypeManager->getStorage('cohesion_component')->load($element->getComponentID());

    if ($component) {
      $layout_canvas_instance = $component->getLayoutCanvasInstance();
      if ($layout_canvas_instance) {
        $resolved_field_uuid = $this->resolve_field_uuid_with_path($component, $element, $component_uuid_path, $value);

        if ($this->component_has_form_field($component, $resolved_field_uuid)) {
          $result = $this->format_field_uuid_result($resolved_field_uuid, $repeater_key, $index);
        }
        else {
          $result = '';
        }
  ...
  /**
   * Resolve field UUID using component path or fallback method.
   *
   ...
   * @return string
   */
  private function resolve_field_uuid_with_path($component, $element, $component_uuid_path, $value) {
    $resolved_field_uuid = NULL;

    // Resolve by explicit render path first when available.
    if (!empty($component_uuid_path)) {
      $normalized_component_path = array_values($component_uuid_path);
      if (!empty($normalized_component_path) && $normalized_component_path[0] === $element->getUUID()) {
        array_shift($normalized_component_path);
      }

      if (!empty($normalized_component_path)) {
        $resolved_field_uuid = $this->resolve_nested_component_field_uuid_by_path(
          $component,
    ...
    // Fallback to recursive lookup for legacy contexts.
    if ($resolved_field_uuid === NULL || trim($resolved_field_uuid) === '') {
      $visited_components = [];
      $resolved_field_uuid = $this->resolve_nested_component_field_uuid($component, $value, $visited_components);
    }

    return $resolved_field_uuid;
  }

  /**
   * Format field UUID result with repeater information if needed.
   *
   * @param string $resolved_field_uuid
   * @param string|null $repeater_key
   * @param string|null $index
   *
   * @return string
   */
  private function format_field_uuid_result($resolved_field_uuid, $repeater_key, $index) {
    if (isset($repeater_key) && isset($index)) {
      return $repeater_key . '_' . $index . '_' . $resolved_field_uuid;
    }
    return $resolved_field_uuid;
  }

  /**
   * Resolve a field UUID from a nested component path back to the root
   * component's own form field UUID.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $root_component
   * @param array $component_uuid_path
   * @param string $field_uuid
   *
   * @return string
   */
  private function resolve_nested_component_field_uuid_by_path(
    $root_component,
    array $component_uuid_path,
    $field_uuid,
  ) {
    ...
    $branch = $this->build_component_branch($root_component, $component_uuid_path);
    if (empty($branch)) {
      return $field_uuid;
    }

    return $this->resolve_field_uuid_through_branch($branch, $field_uuid);
  }

  /**
   * Build component branch for field UUID resolution.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $root_component
   * @param array $component_uuid_path
   * @param string $field_uuid
   ...
   * @return array
   */
  private function build_component_branch($root_component, $component_uuid_path) {
    $branch = [];
    $current_component = $root_component;

    foreach ($component_uuid_path as $child_instance_uuid) {
      if (!is_string($child_instance_uuid) || trim($child_instance_uuid) === '') {
        return [];
      }

      $child_component = $this->get_child_component_by_instance_uuid($current_component, $child_instance_uuid);
      if (!$child_component) {
        return [];
      }
  ...
  /**
   * Resolve field UUID through component branch.
   *
   * @param array $branch
   * @param string $field_uuid
   *
   * @return string
   */
  private function resolve_field_uuid_through_branch($branch, $field_uuid) {
    $resolved_field_uuid = $field_uuid;

    for ($i = count($branch) - 1; $i >= 0; $i--) {
      $parent_component = $branch[$i]['parent'];
      $child_instance_uuid = $branch[$i]['child_instance_uuid'];

      $mapped_field_uuid = $this->map_component_child_field_uuid(
        $parent_component,
        $child_instance_uuid,
        $resolved_field_uuid
      );

      if ($mapped_field_uuid !== NULL) {
        $resolved_field_uuid = $mapped_field_uuid;
        continue;
      }

      // If no explicit token mapping exists, allow passthrough when the field
      // exists directly on the parent form.
      if ($this->component_has_form_field($parent_component, $resolved_field_uuid)) {
        continue;
      }
  ...
  /**
   * Load a nested child component from a component canvas element UUID.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $component
   * @param string $child_instance_uuid
   *
   * @return \Drupal\cohesion_elements\Entity\Component|null
   */
  private function get_child_component_by_instance_uuid($component, $child_instance_uuid) {
    $result = NULL;
    $layout_canvas_instance = $component->getLayoutCanvasInstance();

    if ($layout_canvas_instance) {
      foreach ($layout_canvas_instance->iterateCanvas() as $canvas_element) {
        if ($canvas_element->isComponent() && $canvas_element->getUUID() === $child_instance_uuid) {
          $child_component_id = $canvas_element->getComponentID();
          if ($child_component_id) {
            $result = $this->entityTypeManager->getStorage('cohesion_component')->load($child_component_id);
            break;
          }
        }
      }
  ...
  /**
   * Map a field UUID on a specific child component instance model to the
   * parent field UUID token.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $component
   * @param string $child_instance_uuid
   * @param string $model_key_to_match
   *
   * @return string|null
   */
  private function map_component_child_field_uuid($component, $child_instance_uuid, $model_key_to_match) {
    $layout_canvas_instance = $component->getLayoutCanvasInstance();
    if (!$layout_canvas_instance) {
      return NULL;
    }

    $component_model = $layout_canvas_instance->iterateModels('canvas');
    if (!$component_model) {
      return NULL;
    }

    return $this->find_field_uuid_in_component_model($component_model, $child_instance_uuid, $model_key_to_match);
  }

  /**
   * Find field UUID in component model by matching instance and key.
   *
   * @param object $component_model
   * @param string $child_instance_uuid
   * @param string $model_key_to_match
   *
   * @return string|null
   */
  private function find_field_uuid_in_component_model($component_model, $child_instance_uuid, $model_key_to_match) {
    foreach ($component_model as $model_uuid => $model) {
      if ($model_uuid === $child_instance_uuid) {
        return $this->extract_field_uuid_from_model_values($model, $model_key_to_match);
      }
    }
    return NULL;
  }
  ...
  /**
   * Extract field UUID from model values by matching key.
   *
   * @param object $model
   * @param string $model_key_to_match
   *
   * @return string|null
   */
  private function extract_field_uuid_from_model_values($model, $model_key_to_match) {
    foreach ($model->getValues() as $model_key => $model_value) {
      if ($model_key === $model_key_to_match && is_string($model_value)) {
        return $this->extract_field_uuid_from_model_value($model_value);
      }
    }
    return NULL;
  }
  ...
  /**
   * Extract a field UUID token from a canvas model value string.
   *
   * @param string $model_value
   *
   * @return string|null
   */
  private function extract_field_uuid_from_model_value($model_value) {
    // Simplified regex pattern to reduce complexity.
    $pattern = '/(\(|\[)(field\.)([0-9a-f-]{36}|[\s\S]*?\/[\s\S]*?\/[\s\S]*?)(\)|\])/';
    if (preg_match($pattern, $model_value, $matches)) {
      return $matches[3];
    }
  ...
  /**
   * Resolve a field UUID from a nested child component back to the current
   * component's own form field UUID.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $component
   * @param string $field_uuid
   * @param array $visited_components
   *
   * @return string
   */
  private function resolve_nested_component_field_uuid($component, $field_uuid, array &$visited_components = []) {
    $component_id = $component->id();
    $result = $field_uuid;

    if (!isset($visited_components[$component_id])) {
      $visited_components[$component_id] = TRUE;

      // Direct mapping on this component.
      $direct_map = $this->map_component_canvas_field_uuid($component, $field_uuid);
      if ($direct_map !== NULL) {
        $result = $direct_map;
      }
      else {
        $layout_canvas_instance = $component->getLayoutCanvasInstance();
        if ($layout_canvas_instance) {
          $nested_result = $this->resolve_through_nested_components(
            $component,
            $field_uuid,
            $visited_components,
            $layout_canvas_instance
          );
          if ($nested_result !== $field_uuid) {
  ...
  /**
   * Resolve field UUID through nested component wrappers.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $component
   * @param string $field_uuid
   * @param array $visited_components
   * @param object $layout_canvas_instance
   *
   * @return string
   */
  private function resolve_through_nested_components(
    $component,
    $field_uuid,
    array &$visited_components,
    $layout_canvas_instance,
  ) {
    $component_id = $component->id();

    foreach ($layout_canvas_instance->iterateCanvas() as $canvas_element) {
      if (!$canvas_element->isComponent()) {
        continue;
      }

      $child_component_id = $canvas_element->getComponentID();
      if (!$child_component_id || $child_component_id === $component_id) {
        continue;
      }

      $child_component = $this->entityTypeManager->getStorage('cohesion_component')->load($child_component_id);
      if (!$child_component) {
        continue;
      }

      $result = $this->process_child_component_mapping(
        $component,
        $child_component,
        $field_uuid,
        $visited_components
      );
      if ($result !== $field_uuid) {
        return $result;
      }
    }
  ...
  /**
   * Process child component field mapping.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $component
   * @param \Drupal\cohesion_elements\Entity\Component $child_component
   * @param string $field_uuid
   * @param array $visited_components
   *
   * @return string
   */
  private function process_child_component_mapping(
    $component,
    $child_component,
    $field_uuid,
    array &$visited_components,
    ...
    $result = $field_uuid;
    $mapped_from_child = $this->resolve_nested_component_field_uuid($child_component, $field_uuid, $visited_components);

    if ($mapped_from_child !== $field_uuid) {
      $mapped_to_parent = $this->map_component_canvas_field_uuid($component, $mapped_from_child);
      if ($mapped_to_parent !== NULL) {
        $result = $mapped_to_parent;
      }
      elseif ($this->component_has_form_field($component, $mapped_from_child)) {
        $result = $mapped_from_child;
      }
    }
  ...
  /**
   * Map a component canvas model key to a field UUID token value.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $component
   * @param string $model_key_to_match
   *
   * @return string|null
   */
  private function map_component_canvas_field_uuid($component, $model_key_to_match) {
    $layout_canvas_instance = $component->getLayoutCanvasInstance();
    if (!$layout_canvas_instance) {
      return NULL;
    }
    ...
    return $this->search_model_for_field_uuid($component_model, $model_key_to_match);
  }

  /**
   * Search component model for matching field UUID.
   *
   * @param object $component_model
   * @param string $model_key_to_match
   *
   * @return string|null
   */
  private function search_model_for_field_uuid($component_model, $model_key_to_match) {
    foreach ($component_model as $model) {
      foreach ($model->getValues() as $model_key => $model_value) {
        if ($model_key === $model_key_to_match && is_string($model_value)) {
          $mapped_field_uuid = $this->extract_field_uuid_from_model_value($model_value);
          if ($mapped_field_uuid !== NULL) {
            return $mapped_field_uuid;
          }
        }
      }
    }
  ...
  /**
   * Check if the given field UUID exists in the component form model.
   *
   * @param \Drupal\cohesion_elements\Entity\Component $component
   * @param string $field_uuid
   *
   * @return bool
   */
  private function component_has_form_field($component, $field_uuid) {
    $layout_canvas_instance = $component->getLayoutCanvasInstance();
    if (!$layout_canvas_instance) {
      return FALSE;
    }

    $component_form_model = $layout_canvas_instance->iterateModels('component_form');
    if ($component_form_model) {
      return isset($component_form_model[$field_uuid]);
    }
  ...
  public function renderStyles($entityId, $entityTypeId) {
    // Memoize results per unique (entityTypeId, entityId) pair for the
    // duration of the request. renderStyles is called once per template
    // instance (e.g. once per list item), so the same component can trigger
    // the full render pipeline — entity load, coh_usage query, and
    // renderer::render() — dozens of times per page for identical inputs.
    // Caching by key avoids redundant work without changing behaviour.
    static $cache = [];
    $cacheKey = $entityTypeId . ':' . $entityId;
    ...
    if (!empty($entity)) {
      ...
      if (!empty($customStylesUsage)) {
        ...
        foreach (array_keys($customStylesUsage) as $customStylesUuid) {
          if (!array_key_exists($customStylesUuid, $uuidToId)) {
            $customStyles = $customStyleStorage->loadByProperties(['uuid' => $customStylesUuid]);
            $customStyle = array_shift($customStyles);
            $uuidToId[$customStylesUuid] = $customStyle ? $customStyle->id() : NULL;
          }
          if ($uuidToId[$customStylesUuid] !== NULL) {
            $customStylesToLoad[] = $uuidToId[$customStylesUuid];
          }
        }
      }
      ...
      if ($customStylesToLoad || $elementStylesLoad) {
        ...
        return $cache[$cacheKey] = $this->renderer->render($templateAttached);
      }
    }
