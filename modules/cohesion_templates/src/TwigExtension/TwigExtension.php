...
class TwigExtension extends \Twig_Extension {
   ...
   */
  public function __construct(RendererInterface $renderer, Token $token, Registry $themeRegistry, TwigEnvironment $twigEnvironment, UuidInterface $uuid, EntityTypeManagerInterface $entity_type_manager, StreamWrapperManager $stream_wrapper_manager, MimeTypeGuesserInterface $extension_mime_type_guesser, ThemeManagerInterface $theme_manager, CohesionUtils $cohesion_utils) {
  ...
  public function renderComponent($componentId, $isTemplate, $_context, $componentFields, $componentInstanceUuid = NULL, $componentContentId = FALSE) {
    ...
    if ($component) {
      ...
      foreach ($componentFields as $uuid => $component_field_id) {
        $componentFieldsValues[$uuid] = $this->buildComponentFields($component_field_id, $uuid, $component_json, $_context, $isTemplate);
      }
  ...
  private function buildComponentFields($component_field_id, $uuid, $component_json, $_context, $isTemplate) {
    ...
    elseif (is_array($component_field_id)) {
      ...
      foreach ($component_field_id as $key => $v) {
        ...
        if(isset($component_json['model'][$key])) {
          $componentFields['#' . $key] = $this->buildComponentFields($v, $key, $component_json, $_context, $isTemplate);
        }else{
