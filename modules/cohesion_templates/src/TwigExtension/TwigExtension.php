...
class TwigExtension extends AbstractExtension {
  ...
  public function drupalBlock($id) {
    ...
    if ($block instanceof Block) {
      ...
      // Merge access cacheability without overwriting existing cache contexts.
  ...
  public function setViewIterate(&$context) {
    // Use context-scoped iteration to avoid global static state. This stores
    // the iterator in the Twig context under '_coh_view_iterate' keyed by
    // a per-view identifier (dom_id when available).
    $key = $this->get_cohesion_view_iterate_key($context);
    if (!isset($context['_coh_view_iterate']) || !is_array($context['_coh_view_iterate'])) {
      $context['_coh_view_iterate'] = [];
    }
    $context['_coh_view_iterate'][$key] = 0;
  ...
  public function getViewIterate(&$context) {
    $key = $this->get_cohesion_view_iterate_key($context);
    if (isset($context['_coh_view_iterate']) && isset($context['_coh_view_iterate'][$key])) {
      return $context['_coh_view_iterate'][$key];
    }
    return 0;
  }
  ...
  public function incrementViewIterate(&$context) {
    ...
    if (!isset($context['_coh_view_iterate'][$key])) {
      $context['_coh_view_iterate'][$key] = 0;
    }
    $context['_coh_view_iterate'][$key] = $context['_coh_view_iterate'][$key] + 1;
  ...
  /**
   * Generate a key to store/retrieve view iteration state in the Twig
   * context. Prefer the view dom_id when available; fall back to view id +
   * display or object hash.
   *
   * @param array &$context
   *   The Twig context passed to the Twig function (by reference).
   *
   * @return string
   *   A string key unique for the view instance within the current render
   *   context.
   */
  private function get_cohesion_view_iterate_key(&$context) {
    $key = 'coh_view';
    if (isset($context['view']) && is_object($context['view'])) {
      $view = $context['view'];
      // If dom_id exists (templates use view.dom_id), prefer it.
      if (isset($view->dom_id) && $view->dom_id) {
        $key .= '_' . $view->dom_id;
      }
      else {
        // Attempt to build a stable key using view id and display when
        // available.
        try {
          $view_id = method_exists($view, 'id') ? $view->id() :
            (property_exists($view, 'id') ? $view->id : NULL);
          $display = property_exists($view, 'display_id') ? $view->display_id :
            (property_exists($view, 'current_display') ? $view->current_display : '');
          if ($view_id) {
            $key .= '_' . $view_id . '_' . $display;
          }
          else {
            // Last resort: object hash.
            $key .= '_' . spl_object_hash($view);
          }
        }
        catch (\Throwable $e) {
          $key .= '_' . spl_object_hash($view);
      ...
      // No view in context: use a default key. This keeps iteration state
      // local to this Twig rendering context.
      $key .= '_default';
    }
    return $key;
  }
