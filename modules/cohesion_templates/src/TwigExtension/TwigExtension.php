...
class TwigExtension extends AbstractExtension {
  ...
  public function tokenReplace($token, $data = NULL, $_context = [], $isTemplate = FALSE) {
    ...
    // Decode entities to fix double-encoding from the token system.
    $token_replacement = Html::decodeEntities($token_replacement);
    // If it's a template, then escape angle brackets so HTML tags render as
    // plain text rather than executing.
    if ($isTemplate) {
      $token_replacement = str_replace(['<', '>'], ['&lt;', '&gt;'], $token_replacement);
    }
