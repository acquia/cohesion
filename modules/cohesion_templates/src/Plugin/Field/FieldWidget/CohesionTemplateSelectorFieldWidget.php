...
#[FieldWidget(
  ...
  label: new TranslatableMarkup("Site Studio template selector widget"),
  description: new TranslatableMarkup("Site Studio template selector widget."),
  field_types: ["cohesion_template_selector"],
)]
class CohesionTemplateSelectorFieldWidget extends WidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    if ($entity->getEntityTypeId() == 'field_config') {
      ...
      if (!is_string($target_entity_type_id) ||
        $target_entity_type_id === '' ||
        !\Drupal::entityTypeManager()->hasDefinition($target_entity_type_id)
      ) {
        return [];
      }
      $target_entity_type = \Drupal::entityTypeManager()->getStorage($target_entity_type_id);
