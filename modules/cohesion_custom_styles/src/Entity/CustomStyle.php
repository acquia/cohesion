...
use Drupal\cohesion_custom_styles\CustomStylesHtmlRouteProvider;
use Drupal\cohesion_custom_styles\CustomStylesListBuilder;
use Drupal\cohesion_custom_styles\Form\CustomStylesDeleteForm;
use Drupal\cohesion_custom_styles\Form\CustomStylesDisableForm;
use Drupal\cohesion_custom_styles\Form\CustomStylesDisableSelectionForm;
use Drupal\cohesion_custom_styles\Form\CustomStylesEnableForm;
use Drupal\cohesion_custom_styles\Form\CustomStylesForm;
...
/**
 * Defines the Custom Styles entity.
 *
 * @ConfigEntityType(
...
#[ConfigEntityType(
  id: 'cohesion_custom_style',
  label: new TranslatableMarkup('Custom style'),
  label_collection: new TranslatableMarkup('Custom styles'),
  label_singular: new TranslatableMarkup('Custom style'),
  label_plural: new TranslatableMarkup('Custom styles'),
  config_prefix: 'cohesion_custom_style',
  entity_keys: [
    'id' => 'id',
    'label' => 'label',
  ...
  handlers: [
    'list_builder' => CustomStylesListBuilder::class,
    'form' => [
      'add' => CustomStylesForm::class,
      'edit' => CustomStylesForm::class,
      'duplicate' => CustomStylesForm::class,
      'extend' => CustomStylesForm::class,
      'delete' => CustomStylesDeleteForm::class,
      'disable' => CustomStylesDisableForm::class,
      'enable' => CustomStylesEnableForm::class,
      'enable-selection' => CohesionEnableSelectionForm::class,
      'disable-selection' => CustomStylesDisableSelectionForm::class,
    ],
    'route_provider' => [
      'html' => CustomStylesHtmlRouteProvider::class,
    ],
  ],
  links: [
    'edit-form' => '/admin/cohesion/styles/cohesion_custom_styles/{cohesion_custom_style}/edit',
...
class CustomStyle extends CohesionConfigEntityBase implements CohesionSettingsInterface {
  ...
  public static function loadParentChildrenOrdered() {
    [$storage, $ids] = self::getParentChildrenOrderedIds();

    return $storage->loadMultiple($ids);
  }

  /**
   * Compute the parent/children/weight ordered entity IDs without loading
   * the full entities.
   *
   * @return array
   *   A tuple of [EntityStorageInterface $storage, array $ids].
   *
   * @throws \Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  protected static function getParentChildrenOrderedIds(): array {
    $ids = [];
    ...
    if ($parent_ids) {
      $parents = $storage->loadMultiple($parent_ids);

      // Batch-load all children of all parents in a single query, keyed by
      // parent class_name, instead of one query per parent.
      $classes_by_entity_id = [];
      foreach ($parent_ids as $entityId) {
        /** @var CustomStyle $parent */
        if ($parent = $parents[$entityId] ?? NULL) {
          $classes_by_entity_id[$entityId] = $parent->getClass();
        }
      }

      $children_by_class = [];
      if ($classes_by_entity_id) {
        $child_ids = $storage->getQuery()
          ->accessCheck(TRUE)
          ->condition('parent', $classes_by_entity_id, 'IN')
          ->sort('label', 'ASC')
          ->sort('weight', 'ASC')
          ->execute();
        ...
        if ($child_ids) {
          ...
          foreach ($child_ids as $childId) {
            ...
            if ($child = $children[$childId] ?? NULL) {
              $children_by_class[$child->getParent()][$childId] = $childId;
            }
          }
        }
      }
      ...
      foreach ($parent_ids as $entityId) {
        ...
        if ($class = $classes_by_entity_id[$entityId] ?? NULL) {
          if (!empty($children_by_class[$class])) {
            $ids += $children_by_class[$class];
          }
        }
      }
    }
  ...
  /**
   * Sort the order of custom styles based on parent/child relationships
   * and weight. This function returns an array of {id}_{configItemId} of
   * custom styles in the correct order for processing and sending to the API.
   *
   * @return array
   * @throws \Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
  ...
  public static function sortOrder(): array {
    // Reorder custom style styles. getConfigItemId() is a pure function of
    // the entity type ID and entity ID, so the ordered IDs can be used
    // directly without loading the full entities.
    [$storage, $ids] = self::getParentChildrenOrderedIds();
    $entity_type_id = $storage->getEntityTypeId();

    $style_order = [];
    foreach ($ids as $id) {
      $style_order[] = $id . '_' . crc32($entity_type_id . '_' . $id);
    }
