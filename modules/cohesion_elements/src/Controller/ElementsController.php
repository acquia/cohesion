...
class ElementsController extends ControllerBase {
  ...
  public function inUse($element) {
    ...
    // If there is stale data, show a message to re-run the element usage.
    if (empty($inUseEntities)) {
      $list = $this->getReRunMessage();
    }
   ...
   * @return array[]
   */
  public function getReRunMessage() {
    return [
      'message' => [
        '#markup' => t('This element was tracked as in use but has been removed, please re-run the element usage'),
      ],
    ];
  }
