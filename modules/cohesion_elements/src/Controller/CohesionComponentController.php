...
class CohesionComponentController extends ControllerBase {
  ...
  /**
   * Return list of all tags for a given element type.
   *
   * @param string $entity_type_id
   *   Type of tag entity, i.e. components, etc.
   *
   * @param bool $bypass_permission_check
   *
   * @return array
   *   Return array of tag objects,
   *   or an empty array if element is not of a supported type.
   */
  public static function getElementTags($entity_type_id, $bypass_permission_check = FALSE) {
    $tags = [];

    try {
      // Load the storage handler for the entity type.
      $storage = \Drupal::entityTypeManager()->getStorage($entity_type_id);

      // Build and execute the entity query.
      $tag_ids = $storage->getQuery()
        ->accessCheck(TRUE)
        ->sort('weight')
        ->execute();

      if (empty($tag_ids)) {
        // Return early if no tags found.
        return $tags;
      }

      // Load entities in a single call.
      $tag_entities = $storage->loadMultiple($tag_ids);

      foreach ($tag_entities as $entity) {
        // Skip entities that don't have group access.
        if ($bypass_permission_check || $entity->hasGroupAccess()) {
          $tags[$entity->id()] = [
            'label' => $entity->label(),
