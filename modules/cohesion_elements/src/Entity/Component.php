...
use Drupal\cohesion_elements\ComponentListBuilder;
use Drupal\cohesion_elements\Form\ComponentDeleteForm;
use Drupal\cohesion_elements\Form\ComponentDisableSelectionForm;
use Drupal\cohesion_elements\Form\ComponentEnableSelectionForm;
use Drupal\cohesion_elements\Form\ComponentForm;
...
#[ConfigEntityType(
  ...
  handlers: [
    ...
    'form' => [
      ...
      'duplicate' => ComponentForm::class,
      'delete' => ComponentDeleteForm::class,
      'enable-selection' => ComponentEnableSelectionForm::class,
      'disable-selection' => ComponentDisableSelectionForm::class,
    ],
    'route_provider' => [
...
class Component extends CohesionElementEntityBase implements CohesionSettingsInterface, CohesionElementSettingsInterface, ContentIntegrityInterface {
  ...
  const TAG_ENTITY_TYPE_ID = 'cohesion_component_tag';
  ...
  /**
   * Function to get tag information from the tag entities.
   *
   * @param \Drupal\cohesion_elements\Entity\ComponentTag[] $tag_entities
   *   Array of ComponentTag objects.
   * @param bool $detailed
   *   Whether to return detailed information (ID, label, class) or just IDs.
   *
   * @return array
   *   Array of tag details or IDs, depending on the $detailed parameter.
   */
  public static function getTagDetails(array $tag_entities, bool $detailed = TRUE): array {
    return array_filter(
      array_map(
        function ($entity) use ($detailed) {
          if (!$entity instanceof ComponentTag || !$entity->id()) {
            return NULL;
          }
          return $detailed
            ? [
  ...
  /**
   * Get tag details by an array of tag IDs.
   *
   * @param array $tag_ids
   *   An array of tag IDs.
   *
   * @return array
   *   An array of tag details, where each tag ID maps to its class and label.
   */
  public function getTagDetailsByIDs() {
    $tag_details = [];
    $tag_ids = $this->getTag();

    // Load tag entities based on provided tag IDs.
    $tags = \Drupal::entityTypeManager()->getStorage('cohesion_component_tag')->loadMultiple($tag_ids);

    // Iterate through loaded tags and fetch details.
    foreach ($tags as $tag) {
      if ($tag) {
        $tag_details[$tag->id()] = [
          'label' => $tag->label(),
