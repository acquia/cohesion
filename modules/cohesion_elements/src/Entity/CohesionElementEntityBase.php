...
abstract class CohesionElementEntityBase extends CohesionConfigEntityBase implements CohesionSettingsInterface, CohesionElementSettingsInterface {
  ...
  /**
   * Get the type of tag entity for this entity.
   *
   * @return mixed
   */
  public function getTagEntityTypeId() {
    return get_called_class()::TAG_ENTITY_TYPE_ID;
  }
  ...
  /**
   * Get the entity tag referenced in this entity's 'tag' field.
   *
   * @return array|\Drupal\Core\Entity\EntityInterface[]
   *   Returns an array of tag entities or an empty array if none are found.
   */
  public function getTagEntity() {
    // Get the tag entity type storage.
    try {
      $storage = \Drupal::entityTypeManager()->getStorage($this->getTagEntityTypeId());
    }
    catch (\Throwable $e) {
      return [];
    }
    ...
    // If $tags is an array, load all entities at once.
    if (is_array($tags) && !empty($tags)) {
      return $storage->loadMultiple($tags) ?: [];
    }

    // If $tags is a single value, load the entity and return as array.
    if ($tags && $entity = $storage->load($tags)) {
      return [$entity];
    }
