  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($categories = $this->entityTypeManager->getStorage($category_type_id)->loadMultiple($categories_query->execute())) {
      foreach ($categories as $category) {
        ...
        $this->query = $this->entityTypeManager->getStorage($this->entityType->id())->getQuery()
          ->accessCheck(TRUE)
        ...
        if ($custom_components = $this->customComponentsService->getComponentsInCategory(ComponentCategory::load($category->id()))) {
          // Filter custom components by tags if tag filter is set.
          if ($keys = $this->currentRequest->query->all('tags')) {
            ...
            foreach ($custom_components as $custom_component) {
              ...
              if (array_intersect($customComponentTagIds, $keys)) {
                $filteredCustomComponents[] = $custom_component;
              }
            }

            $custom_components = $filteredCustomComponents;
          }
        ...
        $count = $this->query->count()->execute() + count($custom_components);
