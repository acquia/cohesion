...
class TagForm extends EntityForm {
  ...
  /**
   * @var \Drupal\cohesion_elements\Entity\ElementTagBase*/
  protected $entity;
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    // COHESION_ELEMENTS_COHESION_COMPONENT_TAG_COUNT.
    $options = [];
    for ($i = 1; $i <= COHESION_ELEMENTS_COHESION_COMPONENT_TAG_COUNT; $i++) {
  ...
  public function save(array $form, FormStateInterface $form_state, $redirect = NULL) {
    ...
    if ($this->entity->isNew()) {
      ...
      if ($ids = $query->execute()) {
        ...
        // There are no tags, so safely set weight to 1.
