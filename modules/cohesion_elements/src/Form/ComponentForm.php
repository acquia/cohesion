...
class ComponentForm extends ElementBaseForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $tags = $this->entity->getTag();
    $tagsDefaultValue = !empty($tags) ? ComponentTag::loadMultiple((array) $tags) : [];

    $form['details']['tag'] = [
      '#type' => 'entity_autocomplete',
      '#target_type' => 'cohesion_component_tag',
      '#title' => 'Tags',
      '#description' => t('Add existing tags to a component, if a tag does not exist it will be created. Comma separate tags to add multiple.'),
      '#tags' => TRUE,
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($json_values = $form_state->getValue('json_values')) {
      $canvasInstance = new LayoutCanvas($json_values);
