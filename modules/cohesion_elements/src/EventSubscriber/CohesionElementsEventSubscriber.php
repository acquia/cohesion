...
class CohesionElementsEventSubscriber implements EventSubscriberInterface {
  ...
  private function addHelperListUrl($event) {
    ...
    $event->addAdminUrls('helpers-elements-list', [
      'url' => $url,
      'method' => 'GET'
    ]);
  }
  ...
  private function functionGetAccessList(&$route_params) {
    $entity = $this->getCurrentPathEntityId();
   ...
   * @param $event
   */
  private function addElementsListUrl($event) {
    $route_params = [
      'group' => 'elements',
      'withcategories' => TRUE,
      'entityTypeId' => $this->getCurrentPathEntityId(),
    ];

    $url = Url::fromRoute('cohesion_website_settings.elements', $route_params)->toString();

    $event->addFrontEndUrl('elements-list', [
      'url' => $url,
      'method' => 'GET'
    ]);

    $event->addAdminUrls('elements-list', [
      'url' => $url,
      'method' => 'GET'
    ]);
  }

  private function getCurrentPathEntityId() {
    $params = $this->currentRouteMatch->getParameters();
    foreach ($this->currentRouteMatch->getParameters() as $param_key => $param) {
      if ($param instanceof EntityInterface) {
        return $param->getEntityTypeId();
