...
#[FieldWidget(
  ...
  label: new TranslatableMarkup("Site Studio layout canvas"),
  field_types: ["entity_reference_revisions", "cohesion_entity_reference_revisions"],
...
class CohesionLayoutBuilderWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Validates the cohesion layout builder element.
   *
   * @param array $element
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  public function validateElement(array &$element, FormStateInterface $form_state, array &$complete_form) {
    ...
    // Build the value array ensuring target_id is set before entity to maintain
    // consistency with EntityReferenceRevisionsItem validation.
    $value = [];
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    ...
    foreach ($values as $delta => &$item) {
      if (isset($item['entity'])) {
        ...
        // Keep item IDs in sync with the final entity object to satisfy
        // strict entity reference validation when both IDs and entity exist.
        if (!$entity->isNew()) {
