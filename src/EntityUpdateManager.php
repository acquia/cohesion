...
class EntityUpdateManager {
  ...
  /**
   * Determine if any updates in scope require an API call.
   *
   * @param EntityUpdateInterface $entity
   * @return bool
   */
  public function entityNeedsApiCall(EntityUpdateInterface &$entity) {
    if ($entity->isNew() || is_null($entity->getLastAppliedUpdate())) {
      // New entity - flag for API call.
      return TRUE;
    }

    $last_applied = $entity->getLastAppliedUpdate();
    $all_plugins = $this->getAllPluginDefinitions($last_applied);

    $requiresAPI[] = FALSE;
    foreach ($all_plugins as $plugin_definition) {
      if (!isset($plugin_definition['requiresAPI'])) {
        $requiresAPI[] = TRUE;
      }
      else {
        $requiresAPI[] = $plugin_definition['requiresAPI'];
      }
    }

    // If requiresAPI has been true at any point we must call the API.
    if (in_array(TRUE, $requiresAPI) && isset($all_plugins[$last_applied]) && end($all_plugins) != $all_plugins[$last_applied]) {
      return TRUE;
    }
   ...
   *
   * @param string|null $lastApplied
   *
   * @return array
   */
  public function getAllPluginDefinitions(?string $lastApplied = NULL) {
    ...
    $key = array_search($lastApplied, array_keys($definitions), TRUE);
    if ($key !== FALSE) {
      return array_slice($definitions, $key, NULL, TRUE);
    }
