...
/**
 * Generic Acquia Cohesion event subscriber  .
 */
class CohesionEventSubscriber implements EventSubscriberInterface {
  ...
  public function onConfigSave(ConfigCrudEvent $event) {
    ...
    if($config->getName() == 'system.theme' && $config->getOriginal('default') !== $config->get('default') && $this->cohesionUtils->themeHasCohesionEnabled($config->get('default')) && !theme_get_setting('features.cohesion_build_assets', $config->get('default'))) {
      ...
      $this->messenger->addMessage(t('%theme is using Cohesion and needs assets to be built for it. You may need to perform a <a href=":rebuild">rebuild</a>', ['%theme' => $theme->info['name'], ':rebuild' => Url::fromRoute('cohesion.configuration.rebuild')->toString()]), MessengerInterface::TYPE_WARNING);
    }
    ...
    if(!is_null($config->get('features.cohesion_build_assets')) && $config->get('features.cohesion_build_assets') == 1 && ($config->getOriginal('features.cohesion_build_assets') == 0 || is_null($config->getOriginal('features.cohesion_build_assets')))) {
      $this->messenger->addMessage(t('The theme is now using Cohesion and needs assets to be built for it. You may need to perform a <a href=":rebuild">rebuild</a>', [':rebuild' => Url::fromRoute('cohesion.configuration.rebuild')->toString()]), MessengerInterface::TYPE_WARNING);
    }
  }
