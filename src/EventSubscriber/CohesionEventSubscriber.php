...
class CohesionEventSubscriber implements EventSubscriberInterface {
  ...
  public function onConfigSave(ConfigCrudEvent $event) {
    ...
    // Show message when a theme is installed.
    if ($config->getName() === 'core.extension') {
      $original_themes = $config->getOriginal('theme') ?? [];
      $current_themes = $config->get('theme') ?? [];
      $new_themes = array_diff(array_keys($current_themes), array_keys($original_themes));

      // Reload theme info to ensure new themes are available.
      $this->themeHandler->refreshInfo();

      foreach ($new_themes as $theme) {
        if ($this->cohesionUtils->themeHasCohesionEnabled($theme)) {
          /** @var \Drupal\Core\Extension\ThemeHandlerInterface $themeHandler */
          $theme_info = $this->themeHandler->listInfo()[$theme];
          $this->messenger->addMessage(
            t('%theme is using Site Studio and needs assets to be built for it. You may need to perform a <a href=":rebuild">rebuild</a>.', [
    ...
    // Show message when the default theme changes.
    if ($config->getName() === 'system.theme') {
      $original_default = $config->getOriginal('default');
      $current_default = $config->get('default');
      // Reload theme info to ensure new default is available.
      $this->themeHandler->refreshInfo();
      if ($original_default !== $current_default && !empty($current_default) && $this->cohesionUtils->themeHasCohesionEnabled($current_default)) {
        $theme_info = $this->themeHandler->listInfo()[$current_default] ?? NULL;
        $theme_name = $theme_info ? $theme_info->info['name'] : $current_default;
        $this->messenger->addMessage(
          t('%theme is using Site Studio and needs assets to be built for it. You may need to perform a <a href=":rebuild">rebuild</a>.', [
            '%theme' => $theme_name,
            ':rebuild' => Url::fromRoute('cohesion.configuration.rebuild')->toString(),
