...
class CohesionLayoutRevisionManager {
   ...
   *
   * A revision ID found by buildOrphanQuery() but missing from
   * cohesion_layout_revision is a stray revision_data_table row left behind
   * by prior data drift (e.g. an interrupted deletion) rather than a live
   * revision — the entity API has nothing to load, so such rows are purged
   * directly. See purgeStrayFieldRevisionRow().
   *
   ...
   *
   * @return int
   *   Number of revisions successfully deleted, including purged stray rows.
   */
  public function batchDeleteRevisions(array $revision_ids, string $context = ''): int {
    ...
    foreach ($valid_ids as $revision_id) {
      try {
        ...
        if (!$revision) {
          if ($this->purgeStrayFieldRevisionRow($revision_id)) {
            $deleted++;
            $this->logger->warning('Orphan cleanup: Revision @id had no matching cohesion_layout_revision row; purged the stray cohesion_layout_field_revision row directly.', ['@id' => $revision_id]);
            continue;
          }

          $skipped++;
          if (count($error_details) < self::MAX_ERROR_LOG_DETAILS) {
            $error_details[] = "Revision {$revision_id}: not found (unable to load or purge)";
          }
          $this->logger->warning('Orphan cleanup: Revision @id could not be loaded and could not be purged; skipped', ['@id' => $revision_id]);
          continue;
        }
  ...
  /**
   * Purge a stray cohesion_layout_field_revision row with no live revision.
   *
   * buildOrphanQuery() only excludes revisions still in current use — it
   * never confirms the revision itself still exists in
   * cohesion_layout_revision. On sites where that table has drifted out of
   * sync with its revision_data_table (e.g. a past interrupted deletion),
   * a row can be found by detection yet have no revision for
   * $storage->loadRevision() to load. The entity API cannot clean up a
   * revision that no longer exists, so once we've confirmed there truly is
   * no cohesion_layout_revision row for this ID, delete the stray
   * cohesion_layout_field_revision row directly.
   *
   * @param int $revision_id
   *
   * @return bool
   *   TRUE if a stray row was confirmed and purged.
   */
  protected function purgeStrayFieldRevisionRow(int $revision_id): bool {
    try {
      $exists = (bool) $this->database->select('cohesion_layout_revision', 'clr')
        ->condition('clr.revision', $revision_id)
        ->countQuery()
        ->execute()
        ->fetchField();

      if ($exists) {
        // The revision genuinely exists — loadRevision() failed for some
        // other reason. Do not touch the data.
        return FALSE;
      }

      $rows_deleted = $this->database->delete('cohesion_layout_field_revision')
      ...
      $this->logger->error('Error purging stray field revision row @id: @error', [
        '@id' => $revision_id,
        '@error' => $e->getMessage(),
      ]);
      return FALSE;
