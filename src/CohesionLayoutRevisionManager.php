...
class CohesionLayoutRevisionManager {
  ...
  public function processCronCleanup(): array {
    ...
    if (!$this->should_run_cleanup($config)) {
  ...
  /**
   * Determine if orphan cleanup should run based on configured interval.
   *
   * @return bool
   *   TRUE if cleanup should run, FALSE otherwise.
   */
  public function should_run_cleanup() {
    $config = \Drupal::config('cohesion.settings');
    $cleanup_interval = $config->get('orphan_cleanup_interval', 24);
    $last_run = \Drupal::state()->get('cohesion_orphan_cleanup_last_run', 0);
    ...
    $interval_seconds = $cleanup_interval * 3600;
