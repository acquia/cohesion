...
class LocalFilesManager {
  ...
  /**
   * Whether the stylesheet json is stored in the key value storage or in files.
   *
   * @var bool
   */
  ...
  public function __construct(
    ...
    protected SharedTempStoreFactory $shared_store_factory,
   ...
   * when re-importing.
   */
  public function liveToTemp(): void {
    ...
    if ($this->stylesheetJsonKeyvalue === TRUE) {
      ...
      foreach ($this->cohesionUtils->getCohesionEnabledThemes() as $theme_info) {
        ...
        if (file_exists($from)) {
          try {
            $this->fileSystem->move($from, $to, FileExists::Replace);
          }
   ...
   */
  public function tempToLive(): void {
  ...
  /**
   * Return different filenames depending on if the user is rebuilding.
   *
   * @param string $type
   ...
   * @return string
   */
  public function getStyleSheetFilename(string $type, string $themeId = '', bool $force_clean_filename = FALSE): string {
    ...
    $cohesionUris = [
      ...
      'prefixed' => "prefixed/{$themeFileName}-cohesion-prefixed-ckeditor-stylesheet.css",
    ...
    $tmpUris = [
      ...
      'grid' => "cohesion-responsive-grid-settings.css",
      'icons' => "cohesion-icon-libraries.css",
      'prefixed' => "{$themeFileName}-cohesion-prefixed-ckeditor-stylesheet.css",
   ...
   * This is used because of unpredictable behavior of the /tmp directory on
   * Pantheon and Acquia hosting.
   *
   * @return string
   */
  public function scratchDirectory(): string {
    ...
    if (!file_exists($cohesion_scratch_path)) {
      ...
      $this->fileSystem->mkdir($cohesion_scratch_path, 0777, FALSE);

      // Add a .htaccess file.
      try {
        $this->fileSystem->saveData(FileSecurity::htaccessLines(TRUE), $cohesion_scratch_path . '/.htaccess', FileExists::Replace);
      }
      catch (\Throwable $e) {
        $this->cohesionUtils->errorHandler('Unable to secure directory: ' . $cohesion_scratch_path);
      }
    }
   ...
   */
  public function resetScratchDirectory(): void {
    // Delete the directory.
    if (file_exists($this->scratchDirectory())) {
      $this->fileSystem->deleteRecursive($this->scratchDirectory());
    }
   ...
   * @return bool
   */
  public function deleteFileByURI($uri): bool {
   ...
   * @param $obj
   */
  public function moveTemporaryFiles(&$obj): void {
    if (is_object($obj)) {
   ...
   *
   */
  private function getStylesheetJsonCollectionName($permanent = FALSE): string {
   ...
   */
  public function getStyleSheetJson($theme_name): mixed {

    $original_css_contents = '';

    // If site studio is set to store the stylesheet json to the key value store
   ...
   * @param $theme_id
   */
  public function setStyleSheetJson($stylesheet_json_content, $theme_id): void {
    if ($this->stylesheetJsonKeyvalue === TRUE) {
      $stylesheet_json_value = $this->getKeyValueStore()->get($this->getStylesheetJsonCollectionName());
      ...
      $this->fileSystem->saveData($stylesheet_json_content, $stylesheet_json_path, FileExists::Replace);
    }
  }
   ...
   * @return mixed
   */
  public static function getKeyValueStylesheetJson(): mixed {
