...
class LocalFilesManager {
  ...
  public function liveToTemp() {
    ...
    if ($this->stylesheetJsonKeyvalue === TRUE) {
      ...
      foreach ($this->cohesionUtils->getCohesionEnabledThemes() as $theme_info) {
        ...
        if (file_exists($from)) {
          try {
            \Drupal::service('file_system')->move($from, $to, FileExists::Replace);
          }
  ...
  public function scratchDirectory() {
    ...
    if (!file_exists($cohesion_scratch_path)) {
      ...
      // Add a .htaccess file.
      try {
        \Drupal::service('file_system')->saveData(FileSecurity::htaccessLines(TRUE), $cohesion_scratch_path . '/.htaccess', FileExists::Replace);
      }
      catch (\Throwable $e) {
  ...
  public function setStyleSheetJson($stylesheet_json_content, $theme_id) {
    if ($this->stylesheetJsonKeyvalue === TRUE) {
      ...
      \Drupal::service('file_system')->saveData($stylesheet_json_content, $stylesheet_json_path, FileExists::Replace);
    }
  }
