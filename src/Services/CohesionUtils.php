...
class CohesionUtils {
  ...
  public function getCohesionTemplateOnlyEnabledThemes() {
    $themes = [];
    foreach ($this->themeHandler->listInfo() as $theme_info) {
      if (\Drupal::service('Drupal\Core\Extension\ThemeSettingsProvider')->getSetting('features.layout_canvas_field', $theme_info->getName())) {
  ...
  public function getPayloadForLayoutCanvasDataMerge($entity) {
    ...
    foreach ($components as $component) {
      $tag_entities = !empty($component->getTag()) ? $component->getTagEntity() : [];
      $components_data[$component->id()] = array_merge([
        ...
        'tags' => $tag_entities ? Component::getTagDetails($tag_entities) : [],
      ...
      try {
        ...
        foreach ($component_ids as $component_id) {
          if (isset($custom_components[$component_id])) {
            $custom_component = $custom_components[$component_id];
            $components_data[$component_id] = array_merge([
              'title' => $custom_component['name'],
              'category' => $custom_component['category']->get('class'),
              'tag' => isset($custom_component['tag']) ? Component::getTagDetails($custom_component['tag'], FALSE) : [],
