...
class CohesionUtils {
  ...
  public function processFieldValues($fieldValue, $model, $default = NULL) {
    if (!$model->getProperty(['settings', 'type'])) {
      if ($model->getProperty(['settings', 'schema', 'type']) === 'string' && (!$model->getProperty(['settings', 'schema', 'escape']) || $model->getProperty(['settings', 'schema', 'escape']) === TRUE)) {
      ...
      switch ($model->getProperty(['settings', 'type'])) {
        ...
        case 'cohTextarea':
          if (!$model->getProperty(['settings', 'schema', 'escape']) || $model->getProperty(['settings', 'schema', 'escape']) === TRUE) {
            $fieldValue = Html::escape($fieldValue);
          }
          break;
