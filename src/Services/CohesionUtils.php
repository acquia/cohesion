...
class CohesionUtils {
   ...
   * @param $value
   */
  public function processTokenForApi(&$value): void {
    if (is_string($value)) {
      $token_info = $this->token->getInfo();

      if ($found_tokens = $this->token->scan($value)) {
        foreach ($found_tokens as $context => $token_group) {
          if (isset($token_info['types'][$context])) {
            foreach ($token_group as $name => $token) {
              if (!isset($token_info['tokens'][$context][$name])) {
                // Keep chained/dynamic tokens (e.g. field-based tokens) since
                // many valid variants are not exposed in token info metadata.
                if (!str_contains($name, ':')) {
                  // Remove invalid tokens from the value.
