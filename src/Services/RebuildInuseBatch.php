...
class RebuildInuseBatch {
   ...
   * @return void
   */
  public function run($in_use_list, $changed_entities = [], $syncRebuild = FALSE) {
    ...
    $operations = $this->getOperations($in_use_list, $changed_entities, $syncRebuild);
    if (empty($operations)) {
      return;
    }
    ...
    \Drupal::moduleHandler()->loadInclude('cohesion_website_settings', 'module');
    batch_set($batch);
  }
   ...
   */
  public function getOperations($in_use_list, $changed_entities = [], $syncRebuild = FALSE) {
    $operations = [];
    ...
    foreach ($changed_entities as $entity) {
      ...
      $operations[] = [
        '_resave_entity', [$entity, TRUE, $syncRebuild],
      ];

      $operations[] = [
        '_cohesion_styles_bulk_save', [[$entity->id()], $entity->getEntityTypeId(), FALSE],
      ];
    }
    ...
    foreach ($in_use_by_type as $entity_type_id => $uuids_list) {
      ...
      if($entity_type instanceof ConfigEntityTypeInterface) {
        foreach ($uuids_list as $uuids) {
          ...
          if (!empty($ids)) {
            $operations[] = [
              '_resave_config_entity', [$ids, $entity_type_id, FALSE, TRUE],
            ];
          }
        }
  ...
  public static function finishedCallback($success, $results, $operations) {
    ...
    // Check for authentication errors first - these are fatal and require
    // immediate attention from the operator.
    if (!empty($results['auth_error'])) {
      $messenger->addError($results['error'] ?? t('Entities in use rebuild failed due to authentication error. Please check your Site Studio API credentials.'));
      return;
    }

    // Check for other errors stored in results.
