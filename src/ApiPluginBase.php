...
abstract class ApiPluginBase extends PluginBase implements ApiPluginInterface, ContainerFactoryPluginInterface {
  ...
  public function __construct(
    ...
    protected LocalFilesManager $localFilesManager,
    protected ConfigInstallerInterface $configInstaller,
    protected CohesionUtils $cohesionUtils,
    ...
    // Initialize the logger with the specific channel
  ...
  /**
   * Get the full response from the API call.
   *
   * @return array|null
   *   The full API response array including code, request_id,
   *   is_auth_error, etc.
   */
  ...
  public function isAuthError() {
    return !empty($this->response['is_auth_error']);
  }
  ...
  public function prepareData($attach_css = TRUE) {
    ...
    foreach ($this->cohesionUtils->getCohesionEnabledThemes() as $theme_info) {
      ...
      if ($this->themeHandler->getDefault() == $theme_info->getName() ||
        \Drupal::service('Drupal\Core\Extension\ThemeSettingsProvider')->getSetting('features.cohesion_build_assets', $theme_info->getName())) {
        if ($attach_css == TRUE) {
   ...
   * @return array
   */
  protected function processStyleDiff($currentStyles, $cssDiff): array {
    // Merge both added and updated styles
    $stylesAdded = $cssDiff['added'] ?? [];
    $stylesUpdated = $cssDiff['updated'] ?? [];

    // Process custom styles only if present.
    if ($this->moduleHandler->moduleExists('cohesion_custom_styles')
      && isset($currentStyles['cohesion_custom_style'])
      && (!empty($stylesAdded['cohesion_custom_style']) || !empty($stylesUpdated['cohesion_custom_style']))
    ) {
      $currentStyles['cohesion_custom_style'] = $this->processCustomStyles(
        $currentStyles['cohesion_custom_style'],
        $stylesAdded['cohesion_custom_style'] ?? [],
        $stylesUpdated['cohesion_custom_style'] ?? []
      );
    }

    // Reorder base styles only if present.
    if ($this->moduleHandler->moduleExists('cohesion_base_styles')
      && isset($currentStyles['cohesion_base_styles'])
    ) {
      // Order base styles - first default styles (non-"base_" prefixed) and
      // then user-created styles ("base_" prefixed).
      $currentStyles['cohesion_base_styles'] = BaseStyles::reorderStyles($currentStyles['cohesion_base_styles']);
    }

    // Merge added and updated styles only if not empty.
    if (!empty($stylesAdded)) {
      $currentStyles = $this->array_merge_recursive_distinct($currentStyles, $stylesAdded);
    }
    if (!empty($stylesUpdated)) {
      $currentStyles = $this->array_merge_recursive_distinct($currentStyles, $stylesUpdated);
    }

    foreach ($stylesAdded as $styleTypeKey => $styleAdded) {
      if (isset($currentStyles[$styleTypeKey])) {
        $currentStyles[$styleTypeKey] = $styleAdded + $currentStyles[$styleTypeKey];
      } else {
        $currentStyles[$styleTypeKey] = $styleAdded;
      }
    }

    // If a styling is being deleted.
    if (!empty($cssDiff['deleted'])) {
      $currentStyles = $this->processDeletedStyles($currentStyles, $cssDiff);
    }
  ...
  /**
   * Process the custom styles section of the styles diff from the API,
   * merging in any added or updated styles and re-ordering based on the
   * sortOrder() method.
   *
   * @param $currentStyles
   * @param $stylesAdded
   ...
   * @return array
   */
  protected function processCustomStyles($currentStyles, $stylesAdded, $stylesUpdated): array {
    // Merge updated and added styles, prioritizing updated over added.
    $mergedStyles = $stylesUpdated + $stylesAdded;

    // Overwrite or add merged styles to current styles.
    foreach ($mergedStyles as $key => $value) {
    ...
    $sort_order = [];
    try {
      $sort_order = CustomStyle::sortOrder();
    } catch (InvalidPluginDefinitionException | PluginNotFoundException $e) {
      $this->logger->error('Error ordering custom styles: @error', [
        '@error' => $e->getMessage(),
      ]);
    }
    ...
    foreach ($sort_order as $key) {
      if (isset($currentStyles[$key])) {
        $ordered[$key] = $currentStyles[$key];
      }
    }
    // Append any remaining items not in $sort_order.
    foreach ($currentStyles as $key => $value) {
   ...
   * @return array
   */
  protected function processDeletedStyles($currentStyles, $cssDiff): array {
    foreach ($currentStyles as $k => $currentStyle) {
      if (isset($cssDiff['deleted']['styles'][$k])) {
        foreach ($cssDiff['deleted']['styles'][$k] as $key => $deletedStyle) {
          unset($currentStyles[$k][$key]);
        }
      }
    }
  ...
  private function saveCssEntry($css, $destination) {
    ...
    try {
      $this->fileSystem->saveData($cssData, $destination, FileExists::Replace);
    }
    catch (\Throwable $e) {
      $this->messenger->addError(t('The file could not be created.'));
    }
  ...
  public function send() {
    ...
    // Store the response in a static variable so batch callbacks can check
    // for auth errors.
    if ($this->response !== NULL) {
      $cohesion_last_api_response = &drupal_static('cohesion_last_api_response');
      $cohesion_last_api_response = $this->response;
    }
    ...
    if ($this->response && floor($this->response['code'] / 200) == 1) {
      ...
      if (isset($this->entity)) {
        ...
        $entityType = $this->entity->getEntityTypeId() ?? NULL;
      }

      // Dispatch RequestExceptionEvent with debug data.
      $event = new RequestExceptionEvent(
        ...
        $this->response['exception_message'] ?? '',
        $this->response['data'],
        $this->response['request_duration'],
        $entityId,
        $entityType,
      );
      \Drupal::service('event_dispatcher')->dispatch($event, 'cohesion.request_exception');
    }
  ...
  /**
   * Find key prefix in both 'added' and 'updated' sections for styles diff.
   */
  protected function findFirstKeyByPrefix($stylesDiff, $prefix) {
    foreach (['added', 'updated'] as $section) {
      if (isset($stylesDiff[$section]['cohesion_website_settings']) && is_array($stylesDiff[$section]['cohesion_website_settings'])) {
        foreach ($stylesDiff[$section]['cohesion_website_settings'] as $k => $v) {
  ...
  protected function processWebsiteSettingsDiff($stylesDiff, $key, $StyleSheetFilename, $name) {
    ...
    $updateStyles = $this->findFirstKeyByPrefix($stylesDiff, $key);

    try {
      $this->fileSystem->saveData($updateStyles, $destination, FileExists::Replace);
      if (!$runningDx8Batch) {
