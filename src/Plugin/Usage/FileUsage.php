...
#[Usage(
  id: "file_usage",
  name: new TranslatableMarkup("File usage"),
  entity_type: "file",
  scannable: FALSE,
...
class FileUsage extends UsagePluginBase {
  ...
  public function __construct(
    ...
    // Iterate over keys to safely unset during iteration.
    foreach (array_keys($this->wrappers) as $key) {
      try {
        $this->wrappers[$key]['instance'] = $stream_wrapper_manager->getViaUri($key . '://');
      } catch (\InvalidArgumentException $e) {
        // Skip wrappers that require file extension (e.g., ExtensionStreamBase
        // wrappers like module:// or theme://). These wrappers cannot be
        // instantiated with just a scheme.
  ...
  private function convertBasepathsToUris(&$data) {
    foreach ($this->wrappers as $key => $wrapper) {
      // Ensure the wrapper instance exists before accessing it.
      if (isset($wrapper['instance']) && method_exists($wrapper['instance'], 'getDirectoryPath')) {
