...
#[Usage(
  ...
  name: new TranslatableMarkup("Entity view display usage"),
...
class EntityViewDisplayUsage extends UsagePluginBase {
  ...
  public function getScannableData(EntityInterface $entity) {
    ...
    foreach ($all_view_modes as $view_mode) {
      if ($target_entity_type_id == $view_mode->getTargetType()) {
    ...
    if (!is_string($target_entity_type_id) ||
      ...
      !$this->entityTypeManager->hasDefinition($target_entity_type_id)
    ) {
      return $scannable;
    }
    ...
    if (!is_string($bundle_entity_type) ||
      $bundle_entity_type === '' ||
      !$this->entityTypeManager->hasDefinition($bundle_entity_type)
    ) {
      return $scannable;
    }
