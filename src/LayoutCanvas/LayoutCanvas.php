    ...
    foreach ($this->iterateCanvas() as $element) {
      ...
      if ($element->getModel()) {
        foreach ($element->getModel()->getValues() as $key => $value) {
          ...
          elseif ($withMediaReferences) {
            if (is_string($value) && preg_match(self::MEDIA_REFERENCE_REGEX, $value)) {
              if ($reference = $this->decodeMediaReferenceToken($value)) {
                $references[] = [
                  'entity_type' => $reference[1],
                  'entity_id' => $reference[2],
                ];
              }
            }
          }
          elseif ($withLinksReferences) {
            if (is_string($value) && preg_match(self::LINK_REFERENCE_REGEX, $value)) {
              if ($link = $this->decodeLinkReference($value)) {
                $links[] = [
                  'entity_type' => $link[0],
                  'entity_id' => $link[1],
                ];
