...
class SystemSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // Orphan cleanup settings group.
    $form['orphan_cleanup_accordion'] = [
      '#type' => 'details',
      '#collapsible' => TRUE,
      '#collapsed' => FALSE,
      '#title' => $this->t('Orphaned revision cleanup settings'),
      '#weight' => -95,
      'delete_orphaned_revisions_on_cron' => [
        '#type' => 'checkbox',
        '#title' => $this->t('Delete orphaned revisions on cron?'),
        '#description' => $this->t('When enabled, Site Studio will automatically clean up orphaned layout canvas revisions during daily cron runs. This helps keep the database clean and reduces storage usage.'),
        '#default_value' => $this->cohesionSettings->get('delete_orphaned_revisions_on_cron') ?? FALSE,
        '#ajax' => [
          'callback' => '::updateOrphanCleanupSettings',
          'wrapper' => 'edit-orphan-cleanup-time-wrapper',
        ],
      ],
      'orphan_cleanup_time_wrapper' => [
        '#prefix' => '<div id="edit-orphan-cleanup-time-wrapper">',
        '#suffix' => '</div>',
      ],
      '#open' => 'panel-open',
    ];

    // Add time configuration if cleanup is enabled. When enabled the field is
    // required; when disabled, it is hidden (wrapper left empty).
    $cleanup_enabled = $form_state->getValue('delete_orphaned_revisions_on_cron') ?? $this->cohesionSettings->get('delete_orphaned_revisions_on_cron', FALSE);
    if ($cleanup_enabled) {
      $form['orphan_cleanup_accordion']['orphan_cleanup_time_wrapper'] = $this->buildOrphanCleanupTimeSettings(TRUE, $form_state);
    }
    else {
      $form['orphan_cleanup_accordion']['orphan_cleanup_time_wrapper'] = [
        '#prefix' => '<div id="edit-orphan-cleanup-time-wrapper">',
        '#suffix' => '</div>',
      ];
    }

    // Should the Link element Link directly to the source field file.
    $form['link_element_media_accordion'] = [
      '#type' => 'details',
      '#collapsible' => TRUE,
      '#collapsed' => FALSE,
      '#title' => $this->t('Link element Media settings'),
      '#weight' => 10,
      'link_element_media' => [
        '#type' => 'checkbox',
        '#title' => $this->t('Link to media file'),
        '#required' => FALSE,
        '#default_value' => \Drupal::config('cohesion.settings')->get('link_element_media') ?: FALSE,
        '#wrapper_attributes' => ['class' => ['clearfix']],
        '#attributes' => [
          'class' => [],
        ],
        '#description' => $this->t('If enabled media referenced by the link element will link directly to the referenced file, rather than the media entity itself.'),
      ],
      '#open' => 'panel-open',
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    // Validate orphan cleanup settings
    $cleanup_enabled = $form_state->getValue('delete_orphaned_revisions_on_cron');
    if ($cleanup_enabled) {
      $cleanup_interval = $form_state->getValue('orphan_cleanup_interval');
      if (empty($cleanup_interval)) {
        $form_state->setErrorByName('orphan_cleanup_interval', $this->t('Cleanup interval field is required when orphan cleanup is enabled.'));
      }
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->cohesionSettings->set('delete_orphaned_revisions_on_cron', $form_state->getValue('delete_orphaned_revisions_on_cron'));
    $this->cohesionSettings->set('orphan_cleanup_interval', $form_state->getValue('orphan_cleanup_interval'));

    // Reset the last run time when interval is changed to start immediately
    \Drupal::state()->delete('cohesion_orphan_cleanup_last_run');
    ...
    $this->cohesionSettings->set('link_element_media', $form_state->getValue('link_element_media'));
  ...
  /**
   * Build orphan cleanup interval settings.
   *
   * @param bool $required
   *   Whether the field is required.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   *   The form state to preserve user input.
   *
   * @return array
   *   Form elements for interval configuration.
   */
  protected function buildOrphanCleanupTimeSettings($required = TRUE, $form_state = NULL) {
    // Try to get value from form state first, then fall back to config
    $interval_config = '';
    if ($form_state && $form_state->getValue('orphan_cleanup_interval') !== NULL) {
      $interval_config = $form_state->getValue('orphan_cleanup_interval');
    } else {
      $interval_config = $this->cohesionSettings->get('orphan_cleanup_interval', '');
    }
    ...
    return [
      ...
      'orphan_cleanup_interval' => [
        '#type' => 'select',
        '#title' => $this->t('Cleanup interval'),
        '#default_value' => $interval_config,
        '#options' => ['' => $this->t('- Select an interval -')] + $this->getIntervalOptions(),
        '#required' => (bool) $required,
        '#empty_option' => $this->t('- Select an interval -'),
        '#empty_value' => '',
        '#name' => 'orphan_cleanup_interval',
      ],
    ];
  }

  /**
   * AJAX callback for updating orphan cleanup settings.
   *
   * @param array $form
   *   The form array.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   ...
   */
  public function updateOrphanCleanupSettings(array $form, FormStateInterface $form_state) {
    $cleanup_enabled = $form_state->getValue('delete_orphaned_revisions_on_cron');

    if ($cleanup_enabled) {
      // When enabled, show and require the interval select.
      return $this->buildOrphanCleanupTimeSettings(TRUE, $form_state);
    }
    // When disabled, hide the interval by returning empty wrapper.
    return [
      '#prefix' => '<div id="edit-orphan-cleanup-time-wrapper">',
      '#suffix' => '</div>',
    ];
  ...
  /**
   * Get interval options for the select element.
   *
   * @return array
   *   Array of interval options.
   */
  protected function getIntervalOptions() {
    return [
      1 => $this->t('Every hour'),
      24 => $this->t('Every 24 hours (daily)'),
      48 => $this->t('Every 48 hours (every 2 days)'),
      168 => $this->t('Every 168 hours (weekly)'),
      720 => $this->t('Every 720 hours (monthly)'),
    ];
  }
   ...
   *   The form state.
   */
  public function validateTimeFormat(array $element, FormStateInterface $form_state) {
    $value = $element['#value'];

    if (!empty($value) && !preg_match('/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/', $value)) {
      $form_state->setError($element, $this->t('Please enter a valid time in HH:MM format (24-hour).'));
    }
  }
