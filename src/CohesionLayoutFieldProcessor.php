...
class CohesionLayoutFieldProcessor extends DefaultFieldProcessor {
    ...
    if ($field->getEntity() instanceof CohesionLayout && $field->getName() == 'json_values' && isset($data[0]['value']['#text'])) {
      ...
      foreach ($layout_canvas->iterateCanvas() as $element) {
        ...
        if ($element->isComponent()) {
          ...
          if (!$component_layout_canvas) {
            continue;
          }
          $component_model = $component_layout_canvas->iterateModels('component_form');
          if ($element->getModel()) {
  ...
  public function setTranslations($field_data, FieldItemListInterface $field) {
    // Get the source entity's field to extract the original layout canvas
    // structure. This is necessary because TMGMT 1.17+ creates a new revision
    // before calling setTranslations(), and the translation's field may not
    // have the source json_values content populated yet.
    $source_entity = $field->getEntity()->getUntranslated();
    $source_field = $source_entity->get($field->getName());
    $data = parent::extractTranslatableData($source_field);
    ...
    if ($field->getEntity() instanceof CohesionLayout && $field->getName() == 'json_values' && isset($data[0]['value']['#text'])) {
        ...
        foreach ($field_data as $component_field) {
          foreach ($values as $value) {
            if (is_array($component_field) && isset($component_field['#model_key']) && $component_field['#model_key'] == $value['model_key']) {
              if (!isset($component_field['#translation']['#text']) || !is_string($component_field['#translation']['#text'])) {
                continue;
              }
              $property_path = array_slice($value['model_key'], 1);
              if (is_string($value['value']) && !empty($value['value'])) {
                $model->setProperty($property_path, $component_field['#translation']['#text']);
              }
              elseif (is_object($value['value']) && property_exists($value['value'], 'text') && property_exists($value['value'], 'textFormat')) {
                ...
                $model->setProperty($property_path, $new_value);
              }
            }
          }
        }
      ...
      $translated_json = Json::encode($layout_canvas);

      // Set the translated JSON directly on the entity.
      $entity = $field->getEntity();
      if ($entity instanceof CohesionLayout) {
        $entity->setJsonValue($translated_json);

        // Store the translation in a static cache keyed by entity ID and
        // langcode. This is necessary because the entity_reference_revisions
        // field item may have a different PHP object for the same entity
        // translation, and that object would not have our changes. The
        // CohesionLayout::preSave() method will check this cache and apply
        // the translation if found.
        $langcode = $entity->language()->getId();
        $entity_id = $entity->id();
        if ($entity_id && $langcode) {
          $translation_cache = &drupal_static('cohesion_layout_tmgmt_translations', []);
          $translation_cache[$entity_id][$langcode] = $translated_json;
        }
      }
    }
    else {
