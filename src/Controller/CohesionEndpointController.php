...
use Drupal\cohesion_elements\Controller\CohesionComponentController;
 ...
 *
 * Returns Drupal data to front-end app (views, blocks, node lists, etc).
...
class CohesionEndpointController extends ControllerBase {
   ...
   */
  public function getBreakPointColumns(Request $request): CohesionJsonResponse {
    $results = [];
   ...
   *   The access result
   */
  public function getElementGroupInfoAccess(AccountInterface $account): AccessResult {
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public function getElementGroupInfo(Request $request): CohesionJsonResponse {
    ...
    $categories = ElementsController::getElementCategories($type_map[$entity_type], $this->user->hasPermission('administer components'));
    $element_categories = [];
    ...
    foreach ($entities as $entity) {
      ...
      if ('cohesion_component' === $entity_type) {
        ...
        $element['tags'] = $entity->getTagDetailsByIDs();
      }
   ...
   * @return array
   */
  private function createElementArray($entity, $entity_type): array {
   ...
   * @return array
   */
  private function getPreviewImage($element, $entity_type, $entity): array {
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public function getComponent($uid): CohesionJsonResponse {
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public function getComponentFieldFormModel(string $uid, string $fieldUuid): CohesionJsonResponse {
    $fieldFormModel = [];
    // Load the component.
    if (isset($uid) && ($component_entity = $this->entityTypeManager()->getStorage('cohesion_component')->load($uid))) {
      // Return the json data.
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public function getCanvasUpdateHelper($uid): CohesionJsonResponse {
    // Load the helper.
   ...
   * @throws \Drupal\Component\Plugin\Exception\InvalidPluginDefinitionException
   */
  public function getComponents(Request $request): CohesionJsonResponse {
    $components = [];
    $custom_components = [];
    // Get the uid of the component from the request.
      ...
      foreach ($uids as $uid) {
        if ($component_entity = $this->entityTypeManager()->getStorage('cohesion_component')->load($uid)) {
          ...
          $components[$uid] = array_merge([
            ...
            'tag' => $component_entity->get('tag') ? Component::getTagDetails($component_entity->getTagEntity()) : [],
          ...
          if (array_key_exists($uid, $custom_components)) {
            ...
            $components[$uid] = array_merge([
              ...
              'tag' => isset($custom_components[$uid]['tag'])
                ? Component::getTagDetails($custom_components[$uid]['tag'])
                : [],
            ], $form);
          }
        }
   ...
   * @return \Drupal\cohesion\CohesionJsonResponse
   */
  public function getEntityFields(Request $request): CohesionJsonResponse {
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public function elementSave(Request $request): CohesionJsonResponse {
   ...
   * @return \Drupal\cohesion\CohesionJsonResponse
   */
  public function getSelectOptions(Request $request): CohesionJsonResponse {
   ...
   * @return bool
   */
  private function helperAccessFilter(CohesionElementEntityBase $entity, int $access_elements, bool $isCustomComponentBuilder): bool {
   ...
   * @return bool
   */
  protected function componentListFilter(CohesionElementEntityBase $entity, $type_access = NULL, $bundle_access = NULL): bool {
   ...
   * @return \Drupal\cohesion\CohesionJsonResponse
   */
  public function getEntityPreviewPath(Request $request): CohesionJsonResponse {
   ...
   */
  public function getUriToEntityReference(Request $request): CohesionJsonResponse {
    $file = NULL;
   ...
   * @return \Drupal\cohesion\CohesionJsonResponse
   */
  public function getCategories(Request $request): CohesionJsonResponse {
  ...
  /**
   * Get the list of tags for the save element modal.
   *
   * @param \Symfony\Component\HttpFoundation\Request $request
   *
   * @return \Drupal\cohesion\CohesionJsonResponse
   */
  public function getAllTags(Request $request): CohesionJsonResponse {
    // Get the list of element tags once.
    $element_tags = CohesionComponentController::getElementTags('cohesion_component_tag');

    // Early return if no tags found.
    if (empty($element_tags)) {
      return new CohesionJsonResponse([
        'data' => [],
      ]);
    }
  ...
  /**
   * Fetch a specific group of asset JSON.
   *
   * @param string $group_name
   * @return \Drupal\cohesion\CohesionJsonResponse
   */
  public function getGroupJson(string $group_name): CohesionJsonResponse {
    $module_path = $this->extensionPathResolver->getPath('module', 'cohesion');
    $json_directory = DRUPAL_ROOT . '/' . $module_path . '/js/react-app/assets/json/' . $group_name;

    // First check that the directory exists.
    if (!is_dir($json_directory)) {
      return new CohesionJsonResponse([
        'status' => 'error',
        'data' => ['error' => "JSON directory not found: $json_directory"],
      ], 404);
    }

    $merged_json = [];
    $iterator = new \RecursiveIteratorIterator(
      new \RecursiveDirectoryIterator($json_directory, \FilesystemIterator::SKIP_DOTS)
    );

    foreach ($iterator as $file_info) {
      if ($file_info->isFile() && $file_info->getExtension() === 'json') {
        $data = $this->loadJsonWithIncludes($file_info->getPathname());
        if (is_array($data)) {
          $merged_json[] = $data;
        }
      }
   ...
   * @throws \Exception
   */
  public function loadJsonWithIncludes($filepath): mixed {
    ...
    $resolve = function (&$item, $basePath) use (&$resolve) {
      if (is_array($item)) {
        $module_path = $this->extensionPathResolver->getPath('module', 'cohesion');

        $json_directory = DRUPAL_ROOT . '/' . $module_path . '/js/react-app/assets/json/common/';
        // Recursively search for `loadJson` keys and load the JSON they
        // reference in their place
        if (isset($item['loadJson'])) {
          $item = $this->loadJsonWithIncludes($json_directory . $item['loadJson']);
        }
        else {
          foreach ($item as &$value) {
            $resolve($value, $basePath);
          }
        }
      }
    };
